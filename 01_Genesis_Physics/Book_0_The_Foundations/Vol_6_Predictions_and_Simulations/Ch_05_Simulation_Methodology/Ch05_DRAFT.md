...
But a derivation on paper and a number you can compare with experiment are separated by a canyon. The Waters Field Equations are coupled, nonlinear partial differential equations spanning physical scales from the Waters Below coherence length (η_B ≈ 1.3 × 10⁻¹⁵ m) to the Waters Above coherence length (ξ_A ≈ 3.0 × 10²⁶ m) — forty-one orders of magnitude. Analytical solutions exist for limiting cases: the free membrane vibrations of Volume 1, Chapter 5, the linearized perturbation theory of Volume 5, Chapter 6. But the full coupled system, with its nonlinear self-interactions and cross-coupling between the Waters Above and Waters Below, admits no general closed-form solution.
...
*Physical regime:* Intermediate scales — larger than particle physics (where the membrane vibration module operates) but smaller than cosmological (where the structure formation module operates). Think of it as the "laboratory scale" of the Waters field, analogous to solving Maxwell's equations in a waveguide.
...
This module computes the eigenfrequency spectrum of membrane vibrations — the discrete set of resonance frequencies that, through the relation m_n = ℏω_n/c², predict particle masses. It solves the generalized eigenvalue problem K φ = λ M φ using sparse matrix diagonalization.

*What it answers:* Does the membrane produce a discrete mass spectrum? Do the eigenfrequency ratios match observed particle mass ratios? What is the fundamental mass scale?
...
The modules share a common dependency stack (NumPy, SciPy, Matplotlib) and a common approach to dimensionless variables, but they do not call each other at runtime. Their connection is physical, not computational: the membrane vibration spectrum feeds into the structure formation comparison through the particle content it predicts, and the equilibrium solutions from the field dynamics module inform the initial conditions for both other modules.
...
The reason is arithmetic, not physics. The Waters Below coherence length is η_B ≈ 1.3 × 10⁻¹⁵ m. The Waters Above coherence length is ξ_A ≈ 3.0 × 10²⁶ m. The membrane tension is σ = 6.0 × 10⁹⁸ kg/s². IEEE 754 double-precision floating-point arithmetic carries approximately 15–16 significant decimal digits. Multiplying a number of order 10⁹⁸ by one of order 10⁻¹⁵ and subtracting the result from another number of order 10⁹⁸ would lose all meaningful precision to catastrophic cancellation.
...
|----------|----------|------|-------------------|
| σ (membrane tension) | 6.0 × 10⁹⁸ | kg/s² | Sets wave speed: v = √(σ/μ) ≈ 0.32c |
...
The wave speed on the membrane, v = √(σ/μ) ≈ 9.5 × 10⁷ m/s ≈ 0.32c, is a fundamental derived quantity. It is subluminal (as it must be for consistency with special relativity) and sets the CFL stability condition for the time integration (Section 5.4).
...
*Dirichlet:* φ(0) = φ(L) = 0. Used for the membrane eigenvalue problem, where the membrane is fixed at its boundaries. This is physically motivated by the zone boundary conditions established in Volume 1, Chapter 5, Eqs (1.5.12)–(1.5.18): the Firmament has finite extent, and its vibration modes are standing waves between fixed endpoints.
...
where v_wave is the wave speed on the membrane. In dimensionless units with v_wave = 0.32c (and c = 1 in the dimensionless formulation), this becomes Δt < 0.5 Δx / 0.32 ≈ 1.56 Δx. The implementation uses Δt = 0.001 with Δx = 1/N ≈ 0.004–0.016, giving CFL numbers between 0.06 and 0.25 — well within the stability region.
...
### 5.4.3 The Eigenvalue Problem for Membrane Vibrations

The membrane vibration spectrum is computed not by time evolution but by solving a *static* eigenvalue problem. The membrane equation of motion, after separation of variables (φ(x)e^{iωt}), reduces to:
...
For the membrane vibration spectrum, parameter sensitivity is much higher. The fundamental mass scale m₁ = ℏω₁/c² depends linearly on √(σ/μ), so a factor-of-two error in σ produces a factor-of-√2 error in the predicted masses. Since the membrane tension σ is derived from Planck-scale arguments and not directly measured, this represents a significant source of uncertainty. Chapter 7 addresses this in detail.
...
*1D string eigenfrequencies:* For a string of length L with fixed endpoints, membrane tension σ, and surface density μ, the exact eigenfrequencies are ω_n = (nπ/L)√(σ/μ). The `membrane_vibrations.py` eigensolver reproduces these to relative error < 10⁻¹⁴ for the first 15 modes on a 200-point grid.

*Circular membrane modes:* The eigenfrequencies of a circular membrane with fixed boundary are determined by the zeros of Bessel functions: J_n(k_{nm}R) = 0. The numerical eigenvalues match the Bessel zeros to relative error < 10⁻¹⁰ on a 50 × 50 grid.
...
|-----------|---------------|--------|
| Spatial dimension | 1D and 2D only | Cannot resolve 3D structure formation or fully anisotropic membrane modes |
...
**The methods** (Section 5.4): Second-order centered finite differences for spatial discretization, explicit Euler for time integration, sparse eigenvalue solvers for the membrane spectrum. Every method chosen for transparency and reproducibility.
...
**5.3** (Conceptual) The CFL condition for the Waters field solver involves the wave speed v = √(σ/μ) ≈ 0.32c, not the speed of light c. Explain why the relevant wave speed is the membrane wave speed rather than the speed of light. Under what physical conditions would the CFL condition involve c instead? (*Hint:* Consider the limit where the membrane equation decouples from the Waters field equations.)

**5.4** (Conceptual) Periodic boundary conditions are used for the coupled field evolution, while Dirichlet conditions are used for the membrane eigenvalue problem. Explain the physical motivation for each choice. If you replaced Dirichlet with periodic conditions in the eigenvalue problem, how would the spectrum change? Would the predicted particle masses be affected?
