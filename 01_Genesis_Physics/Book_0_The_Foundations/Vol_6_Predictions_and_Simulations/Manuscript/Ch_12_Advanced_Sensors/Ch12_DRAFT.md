...
> **Convention on sensitivity estimates and their uncertainty.** Every "minimum detectable signal" quoted in this chapter (e.g., $h_\text{min} \approx 10^{-25.5}$ for the MVI in §12.2, $a_\Psi^\text{min} \approx 10^{-24}$ m/s² for the atom interferometer in §12.3) is an **order-of-magnitude engineering estimate**, not a measured instrument specification, and should be read as carrying an uncertainty band of roughly **±0.5 in the base-10 exponent — i.e., a factor of ~3 either way**. That band is not arbitrary: it propagates from the spread in the noise-budget components themselves, which are individually quoted to one significant figure (the dominant term typically sets the floor, and the quadrature sum of the listed thermal, shot, radiation-pressure, seismic, and Waters-coupling power spectral densities carries the sub-dominant terms' uncertainty). Where a signal-to-noise ratio is stated (e.g., SNR ~3 at $10^7$ s integration), the same factor-of-~3 band applies, so a quoted "marginal" detection should be read as spanning "no detection" to "clear detection" depending on where the true noise floor falls within the band. A real engineering proposal would tighten each band by building the noise budget from measured component data; until that is done, the figures here establish *feasibility and ordering*, not guaranteed thresholds. Individual sensitivity numbers below are therefore to be understood with this $\times/{\div}\,3$ band attached even where, for readability, the band is not repeated inline.
