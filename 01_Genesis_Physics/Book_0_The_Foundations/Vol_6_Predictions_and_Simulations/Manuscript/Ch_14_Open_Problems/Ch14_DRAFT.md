...
[FIGURE: Fig 6.14.1 — The Open-Problem Landscape. Scatter plot with domain scope on the horizontal axis (ranging from "single observable" on the left to "framework-wide structural issue" on the right) and severity on the vertical axis (BLOCKER at top, HIGH below it, then MEDIUM, LOW, and INHERITED at bottom). Each of the 29 OPs is plotted as a labeled dot. Color encodes status: red for fully open; yellow for partially addressed in Vols 1–5; green for partially resolved since the original audit or critic report but carried forward for completeness. The BLOCKER (OP-1 spin-½) sits alone at top-center. The five HIGH-severity OPs cluster in the upper-middle band. The MEDIUM and LOW tiers spread across the bottom third, the MEDIUM tier including OP-28 and OP-29 (the two cosmology forward-debts carried in from Vol 5). Eight INHERITED consciousness OPs are visible at far right, flagged as cross-referenced with Ch 13 §13.9.]
...
### 14.5.8  OP-28: Friedmann Density Split Ω_m ≈ 27%, Ω_Λ ≈ 68% — Prediction-vs-Fit Audit (carried from Vol 5 Ch 8)

**What is known.** Vol 5 Ch 8 (the zone cosmological model) reports the present-epoch density split Ω_m ≈ 0.27, Ω_Λ ≈ 0.68 and records it as a match to Planck 2018, logged in the volume's research register as the forward-debt **RT-5.ΩA**. The 27/68 figures also appear in this volume's prediction catalogue as matched predictions P-024 and P-025 (Appendix A). The agreement with observation is real.

**What is missing.** A clean audit separating what is *predicted* from what is *fitted*. The split currently rests on inputs (the Waters-field equation-of-state parameters and the Zone-0/Zone-1 partition) that are calibrated rather than derived from the 6D action. Until that audit is performed, the 27/68 match cannot be cited as a parameter-free prediction; it must be flagged as a constrained fit pending derivation of its inputs from first principles. This is the carried-forward obligation that Vol 5 deferred to Vol 6's open-problems home.

**Resolution path.** Tabulate every numerical input feeding the Friedmann density split, mark each as derived or calibrated, and trace the calibrated ones to a candidate first-principles source (the Waters duality of Vol 1; the Λ_Z0 axiom). Where an input cannot yet be derived, state so explicitly and re-rate P-024/P-025 from "matched prediction" to "matched fit" with the audit flag attached.

**What a resolution would mean.** Either the 27/68 split becomes a genuine zero-parameter prediction (a major result) or its status is honestly downgraded to a constrained fit. Either outcome closes the audit gap that Vol 5 left open.

**Difficulty.** Master's thesis (the audit itself); PhD if the inputs are then derived from first principles. Severity: MEDIUM — the framework's honesty, not its survival, is at stake.

### 14.5.9  OP-29: Two-Phase Creation-Mode Derivation — κ_create vs κ_full (carried from Vol 5 Ch 12)

**What is known.** Vol 5 Ch 12 (the starlight problem and chronology) invokes a two-phase cosmic history: a creation-mode sustaining field κ_create giving way at the Sabbath Boundary to the present sustaining-mode κ_full, logged in the volume's research register as the forward-debt **RT-5.2PH**. The two-phase picture is thermodynamically motivated (a phase transition in the sustaining field κ at Day 7) and is theologically anchored, but the transition is described qualitatively rather than derived.

**What is missing.** A first-principles derivation of the creation-mode equation of state and of the κ_create → κ_full transition from the 6D action — specifically, the values (or bounds) of the sustaining coupling in each regime and the dynamics that carry the cosmos across the boundary. At present κ_create is a posited regime, not a solved one; Vol 5 carries this as an explicit debt to be discharged here.

**Resolution path.** Write the sustaining-field sector of the 6D action with a temperature- or epoch-dependent coupling, solve for the two stable regimes, and show that a phase transition connects them at the conditions Ch 12 assigns to the Sabbath Boundary. Quantify the expansion-rate enhancement during creation-mode and check it against the chronology constraints of Ch 12.

**What a resolution would mean.** The two-phase history becomes a derived feature of the framework rather than a posited one, and the starlight-and-chronology argument of Vol 5 Ch 12 gains a quantitative backbone. Failure to find a consistent two-regime solution would, conversely, force a revision of Ch 12.

**Difficulty.** PhD dissertation. Severity: MEDIUM — it is a derivation-completeness gap in a chapter the framework presents as already coherent, not a blocker for the validated Part A results.
...
The 29 open problems plus three cross-cutting issues defined in this chapter constitute the framework's research program for the next decade and beyond. Choosing which to attack first is an allocation problem, and this section provides the framework's current guidance.
...
OP-3, OP-4, OP-5, OP-7, OP-8, OP-9, OP-10, OP-12, OP-13, OP-14, OP-15, OP-16, OP-18, OP-19, OP-20, OP-21, OP-24, OP-26, OP-27, OP-28, OP-29. This is most of the chapter. Each is a good dissertation topic; none is a program killer. A well-run research group should have 6–10 of these active at any time, with students at various stages.
...
The Genesis Physics framework has 29 open problems: one BLOCKER (OP-1 spin-½ fermions from a bosonic membrane), five HIGH (OP-2 fermion mass spectrum 1000× discrepancy; OP-3 weak/CP structural completeness; OP-4 Higgs completeness; OP-5 Yukawa derivation; OP-6 fine-structure final precision), nine MEDIUM (OP-7 running couplings; OP-8 N-body precision; OP-9 GR observables; OP-10 FTL causality; OP-11 MRG experimental validation; OP-12 life-detection sensitivity; OP-13 QED loops from membrane vacuum; OP-28 Friedmann density-split prediction-vs-fit audit, carried from Vol 5 Ch 8; OP-29 two-phase creation-mode derivation, carried from Vol 5 Ch 12), six LOW (OP-14 PMNS; OP-15 CKM; OP-16 neutrino mass hierarchy; OP-17 MRG η determination; OP-18 BCS from membrane CM; OP-19 ℏ, G, k_B absolute scales), and eight INHERITED from Chapter 13 §13.9 (OP-20 through OP-27). Three cross-cutting issues — the absolute-scale meta-problem, the import-discipline question, and the observational-constraint program — shape the program's overall posture. The critic report and the skeptic analysis have been formally answered, with one originally-FATAL finding resolved (Firmament tension σ) via the `FINE_STRUCTURE_DERIVATION.md` canonical derivation, three CRITICAL findings converted to open problems with experimental paths (replenishment thermodynamics → OP-11/OP-17), and the remaining findings either affirmed (fine-structure constant SOLID; Casimir forces SOLID), resolved (orifice model withdrawn; cosmic capacitor correction adopted), or indexed to specific sub-items of the MRG program. The prioritization matrix identifies the Phase 1 MRG experiment ($150; OP-11) as the single highest-leverage problem in the chapter, and a decade-scale $20–30M program attacking Quadrants I and II is estimated to cost comparable to one mid-sized experimental-physics collaboration.

The 29 OPs are not all uniquely the Genesis Physics framework's. Chapter 15 (Connections to Other Programs) picks up where this chapter ends, specifically by identifying which OPs are shared with string theory, loop quantum gravity, causal sets, and constructor theory, and what the cross-program leverage looks like on each.
...
4. Classify each of OP-1 through OP-29 by whether its resolution would (a) generate a new framework prediction, (b) retire an existing speculation, or (c) tidy an existing derivation. For each category, identify the two OPs with highest impact. (Note that OP-28 and OP-29, the two cosmology debts carried in from Vol 5, are good test cases for category (c) versus (a).)
