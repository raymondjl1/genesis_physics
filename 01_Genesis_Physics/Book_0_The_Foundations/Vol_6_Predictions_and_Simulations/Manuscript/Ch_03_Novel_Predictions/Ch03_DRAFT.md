...
#### Novel-in-principle, untestable-in-practice (P-089, P-090)

A word of calibration before the entries. The two predictions that follow — the vector and scalar polarization modes — are genuine *qualitative* novelties: they exist in the 6D framework and are flatly absent from 4D GR. But they are not strong empirical tests, and we do not present them as such. At the zone architecture's natural compactification scale ($R_{\rm compact} \sim \eta_B \sim 10^{-15}$ m), the amplitude of these modes is suppressed by $(R_{\rm compact}/\lambda_{\rm GW})^2 \sim 10^{-43}$ relative to the standard tensor modes — roughly eighteen orders of magnitude below the most optimistic third-generation detector sensitivity. They become conceivably accessible only under a "large extra dimension" assumption ($R \sim 10^{-6}$ m) that zone architecture does not currently motivate. Read P-089 and P-090, then, as *novel in principle but untestable in practice*: their value is that they sharpen what the framework forbids GR from saying, not that they offer a near-term experimental handle. (The Kaluza-Klein graviton tower, P-091, is a different case — its feasibility is genuinely scale-dependent and can reach MODERATE — so it is presented separately below.)
...
#### Scale-dependent feasibility (P-091)
...
> **Phenomenon:** The Firmament membrane has a ground-state (zero-point) energy that contributes to the cosmological constant. Zone architecture proposes a *mechanism* by which this contribution is suppressed relative to the naive Standard-Model estimate — not by fine-tuning, but because the Firmament membrane's effective mode count is limited by the zone geometry. We are careful not to claim this mechanism *solves* the cosmological-constant problem: the suppression it supplies improves on the Standard Model by many orders of magnitude but, on the mode count estimated below, does not by itself reach the observed value (see the feasibility and falsification notes), so this entry is a defined pathway, not a finished resolution.
...
> The vacuum energy density then scales as $\rho_{\rm vac} \sim (\hbar c / \xi_A)^4 \times N_{\rm eff}$ rather than $(\hbar c / l_{\rm Planck})^4 \times N_{\rm all}$. This replaces the Planck-cutoff sum with a *geometric-cutoff* sum. We must be numerically honest about where that leaves the prediction. Carrying out the cutoff estimate explicitly (Vol 4, Ch 9, Eq (4.9.20)–(4.9.28)) gives $\rho_{\rm vac} \approx 6.76 \times 10^{-6}$ GeV$^4$ — already an improvement of roughly $80$ orders of magnitude over the naive Planck-scale figure, reducing the cosmological-constant discrepancy from $\sim 10^{120}$ to $\sim 10^{41}$. But $6.76 \times 10^{-6}$ GeV$^4$ is still about **41 orders of magnitude larger** than the observed $\rho_{\rm vac,obs} \approx 3.5 \times 10^{-47}$ GeV$^4$. Closing the remaining gap requires an additional suppression factor; Vol 4 Ch 9 §9.6 shows that a factor $\eta_B/\xi_A \approx 4.3 \times 10^{-42}$ would land within $\sim 20\%$ of the observed value (Eq (4.9.30)) — but it states plainly that this is **not yet a derivation**: the suppression exponent is not calculated from first principles and awaits the full 6D Waters–Firmament equilibrium solution. So the mechanism supplies a defined pathway toward the observed value; it does not, on present calculations, reach it. The cosmological-constant problem is not solved here.
...
> **Derivation source:** Research/Papers/membrane_resonance_generator.docx (with the markdown-native simulation in Research/Simulations/mrg_simulation.py); Vol 1, Ch 5 (Firmament vibration modes); Research/Foundations/ENERGY_FRACTIONS_DERIVATION.md. The coupling coefficient between a laboratory oscillator at frequency $f$ and the Firmament membrane mode at frequency $f_n$ is:
