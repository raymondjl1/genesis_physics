...
### 10.7.6  The empirical record — prior vacuum-energy extraction attempts and their null replications

A device that proposes to draw net power from a vacuum-tension reservoir does not arrive in an empty field. It arrives in a field with a long record of **extraordinary claims followed by null replications**, and intellectual honesty requires engaging that record directly rather than stepping around it. The history is summarized and sourced in the supporting Research note `Research/Mathematical_Models/08_Cosmology/08-VACUUM_ENERGY_EXTRACTION_NULL_REPLICATION_HISTORY.md`; the essentials are these.

**The null record.** The NASA Eagleworks "quantum-vacuum thruster" / EmDrive-class measurements (White and co-workers, ~2014–2021) reported small anomalous thrust signals that did **not** survive an independent, higher-fidelity test campaign (Tajmar and co-workers at TU Dresden), which traced the apparent signal to thermal-expansion and mounting/seismic artifacts. The dynamical-Casimir and zero-point-energy extraction analyses of Maclay and collaborators concluded that static Casimir cycles are conservative — closed-cycle work is exactly zero — and that any net extraction requires a genuinely non-conservative, externally driven process. The advanced-propulsion vacuum-energy surveys (Davis and the AIAA/USAF-era reviews) found the vacuum's *paper* energy density enormous but produced **no demonstrated mechanism** for net-positive continuous power. The one positive, replicated result in the cluster — photon production from a relativistically modulated boundary (Wilson et al. 2011, the experimental dynamic Casimir effect) — is precisely *not* free energy: the photons are paid for by the work done modulating the boundary.

**The three lessons we must respect.** (L1) Conservation is never violated: every credible result reduces to an open-system balance, output = reservoir draw − losses, with modulation work explicitly counted; static cycles give zero. (L2) Small anomalous signals are usually artifacts: thermal drift, mounting strain, EMI, and seismic coupling have repeatedly masqueraded as new physics, and a real result must survive the *same* controls that dissolved Eagleworks. (L3) A large vacuum energy density on paper does not imply accessible power — which is exactly why §10.7.1 partitioned the budget into the tiny accessible residual ρ_Λ ≈ 6 × 10⁻¹⁰ J/m³ and the ~10¹¹³ J/m³ structural tension that is *locked* into holding the Firmament open and is *not* harvestable.

**What the MRG concedes, and where it claims to differ.** The MRG does **not** claim to evade L1–L3, and the chapter should not be read as promising that it does. Three honest distinctions, all *conditional on the zone-architecture axioms*:

1. **It is openly an open system.** Unlike most "energy from the vacuum" proposals, the MRG does not posit a closed cycle. Its reservoir is named — the sustaining coupling κ(t), Axiom 5 (§10.10) — and if κ(t) is zero or inaccessible at laboratory scale, the MRG produces no net power. That is a falsifiable outcome, not an escape hatch.
2. **Its differentiator is the DIRECT stage, not the DISRUPT stage.** The DISRUPT (modulated-boundary) stage is ordinary dynamic-Casimir physics — real, replicated, and net-zero by itself. The framework's *additional* claim is that the gravity-aligned magnetic bias of the DIRECT stage (§10.5.4) breaks time-reversal symmetry and rectifies the photon flux into a net direction. This is the step with no external precedent, and is therefore the step that most needs falsification — not the step to lean on.
3. **It must pass the artifact gauntlet that sank Eagleworks.** The framework's strongest honest position here is procedural: the five falsification tests of §10.8 are designed precisely to separate a real effect from the thermal/mounting/EMI artifacts that produced and then dissolved the historical signals. The orientation-dependence test (§10.8.3) is the most discriminating, because no known artifact reproduces a *gravity-aligned* signature.

**Why we expect to do better — and the standing commitment.** The prior nulls were of two kinds: closed-cycle proposals that conservation forbids (Maclay's verdict), and open-but-unmodeled setups whose signal was an uncontrolled artifact (Eagleworks). The MRG is neither — it names its reservoir and it ships the discriminating test suite. That is a reason to *test* it, not a reason to believe it. We therefore state the commitment in advance and without hedging: **if the §10.8 tests fail — no net-energy balance, no magnetic-field dependence, no orientation dependence — the MRG joins the null record, exactly as the EmDrive did, and this chapter's central engineering claim is falsified.** No appeal to "the reservoir was there but inaccessible" will be entertained after the fact; the prediction is the measured output, not the theoretical reservoir.
