...
**Propagated uncertainty.** The two inputs are quoted to two significant figures, so each carries an implied uncertainty of roughly half a unit in the last digit: σ = (6.0 ± 0.05) × 10⁹⁸ kg/(m·s²), a relative uncertainty of 0.83%, and μ = (6.7 ± 0.05) × 10⁸¹ kg/m³, a relative uncertainty of 0.75%. For v = √(σ/μ), the relative uncertainties add in quadrature with a factor of one-half (from the square root):
$$\frac{\delta v}{v} = \frac{1}{2}\sqrt{\left(\frac{\delta\sigma}{\sigma}\right)^2 + \left(\frac{\delta\mu}{\mu}\right)^2} = \frac{1}{2}\sqrt{(0.0083)^2 + (0.0075)^2} \approx 0.56\% \tag{6.7.3a}$$
so the derived wave speed is **v = (0.9975 ± 0.0056) c**. This is the crucial point for the wave-speed convention below: the value c lies *inside* the 1σ band (0.9919c–1.0031c). The 0.25% shortfall of the central value from c is not a statistically significant subluminal result — it is fully accounted for by the input precision, consistent with the framework's claim that v = c exactly. Tightening σ and μ to more significant figures (a Vol 2 derivation task) would shrink this band and is the only way to test whether the central value remains below c.

**Connection to Planck scales.** The two numbers are not arbitrary: σ ≈ 6.0 × 10⁹⁸ kg/(m·s²) is, to order of magnitude, the Planck force per Planck area (a Planck *pressure*, $c^4/G$ per unit area $\sim \rho_{\rm Planck} c^2$), and μ ≈ 6.7 × 10⁸¹ kg/m³ is comparable to the Planck mass distributed over the Planck volume scaled to the membrane thickness η_B — i.e., both are the natural Planck-density-scale quantities (cf. ρ_Planck = 5.15 × 10⁹⁷ kg/m³ in the parameter table). Their ratio being ≈ c² is precisely why the Firmament wave speed comes out at the speed of light: a membrane built at the Planck scale of stiffness and inertia vibrates at the maximal causal speed.
