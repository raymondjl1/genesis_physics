...
*This chapter presents the computational particle mass spectrum predicted by Firmament membrane vibration modes, compares it honestly with observed particle masses, confronts the ~1000× mass discrepancy for light leptons, and identifies the energy harvesting resonance modes that feed into Chapter 10.*
...
| Ch07-002 | Map Firmament membrane vibration modes to particle masses via m_n = ℏω_n/c² | V6-001 | MET (§7.5) |
...
|---------|---------------|
| Firmament membrane wave equation and eigenvalue problem | Vol 1, Ch 5 |
| Waters Above/Below coherence lengths (ξ_A, η_B) | Vol 1, Ch 6 |
| Particle mass from Firmament membrane vibrations (m_n = ℏω_n/c²) | Vol 4, Ch 10 |
| Firmament tension σ and surface density μ derivation | Vol 2, Ch 3 |
...
1. **Why does the Firmament produce discrete particle masses?** — Because a bounded vibrating membrane has discrete eigenfrequencies ω_n, and each frequency maps to a mass through m_n = ℏω_n/c². The same physics as a drumhead producing discrete musical pitches.

2. **Why is the mass scale set by η_B (the Waters Below coherence length)?** — Because η_B sets the effective domain size for the Firmament vibrations — it is the physical scale at which the Firmament boundary conditions apply. The fundamental frequency scales as v/L, and L ~ η_B.

3. **Why is the predicted fundamental mass ~475 MeV/c² instead of 0.511 MeV/c² (the electron)?** — Because ℏv/(c²η_B) ≈ 475 MeV/c² — the natural mass scale of the Firmament is the hadronic scale, not the leptonic scale. This is the ~1000× discrepancy: the Firmament naturally produces proton-scale masses, and generating electron-scale masses requires physics not yet included in the model.
...
6. **Why include the energy harvesting modes?** — Because if the Firmament vibrates, those vibrations carry energy that could in principle be extracted. The resonance modes identified here are the theoretical basis for the Firmament resonance generator concept in Ch 10.
...
|--------|-------|------|-----------|---------------|----------------|------------|---------------------|-----------|
| Fig 6.7.1 | Firmament Vibration Mode Shapes | Diagram | §7.2, after mode equations | First 6 mode shapes for 1D string and circular membrane side by side | Readers need to visualize what vibration modes look like before seeing their masses | Mode numbers n, node positions, displacement amplitude u(x) | Eqs 6.7.1–6.7.3 | Medium |
...
| Fig 6.7.3 | Circular Firmament Spectrum | Plot | §7.3, after circular results | Mass vs mode index for circular membrane, labeled by (n,m) Bessel indices | Non-uniform spacing from Bessel zeros — the spectrum has structure | Bessel indices (n,m), mass (kg) | Eq 6.7.5 | Medium |
...
| Fig 6.7.5 | Predicted vs Observed Particle Masses | Comparison | §7.5, after Table 7.3 | Log-scale comparison: predicted Firmament membrane modes (circles) vs known particle masses (horizontal lines). The ~10¹¹ gap between them is the chapter's central visual. | THE key figure — the honest confrontation with the discrepancy. A reader who sees only this figure understands the chapter's message. | Particle names, mass scale (eV/c²), predicted modes, experimental values | Eq 6.7.6 | Complex |
...
| Fig 6.7.8 | Energy Harvesting Resonance Modes | Diagram | §7.8 | Low-frequency Firmament membrane modes identified as energy harvesting candidates, with coupling mechanism schematic | Bridges to Ch 10 (Firmament membrane resonance generator) | Resonance frequencies, coupling strengths, energy flow arrows | — | Medium |
...
### Section 7.1: Why the Firmament Spectrum Matters
- **Topic sentence:** The Firmament membrane vibration spectrum is where zone architecture makes its most direct — and most troubled — contact with particle physics.
- **"Why" entry point:** Vol 4 Ch 10 predicted that particles ARE Firmament membrane vibration modes. This chapter tests that prediction computationally.
...
- **Topic sentence:** Despite the discrepancy, the Firmament membrane spectrum makes specific, falsifiable predictions.
...
- **Topic sentence:** The same Firmament vibration modes that (attempt to) predict particle masses also predict energy-carrying resonances that could be harvested.
- **"Why" entry point:** If the Firmament vibrates, those vibrations carry energy. This bridges to Ch 10.
- **Key content:** Low-frequency mode identification, coupling mechanism, energy density estimates, connection to Firmament resonance generator.
...
- The energy harvesting modes connect to Ch 10 (Firmament membrane resonance generator). Keep the treatment here brief but complete enough that Ch 10 can reference specific mode numbers and frequencies.
- The wave speed v = √(σ/μ) ≈ 0.9975c — almost exactly the speed of light. This is not coincidental; it reflects the Firmament tension and density being set by Planck-scale physics.
