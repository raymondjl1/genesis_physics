...
That different chapters have used different readings of $\Psi_\mathrm{spirit}$ is a weakness of the framework's consciousness model — not a fatal weakness, because the downstream conclusions of each chapter do not depend on which reading is correct, but a real weakness worth marking. The series adopts **Reading A as default** (most consistent with the quantum-mechanical treatment throughout Vols 1–5) while acknowledging that this choice depends on deriving the Zone 1 Hamiltonian from first principles — a task that remains open. Chapter 14 §14.OP-13.4 identifies the full choice among Readings A/B/C as one of the framework's thesis-buildable open problems. As a methodological commitment, this reading is **pre-registered**: once predictions P-154 through P-158 are run under the reading stated here (or under any reading explicitly declared in advance), switching readings after the fact to rescue a result that the chosen reading would have failed is methodologically impermissible — the reading is fixed before the data, not selected by it.
