...
You now have a complete classical architecture. The zone manifold (Chapter 3) gives you eight nested domains. The 6D metric (Chapter 4) gives you geometry. The Firmament (רָקִיעַ, *rāqîʿaʾ*, 'stretched-out thing'; Chapter 5) gives you a dynamical membrane where $c^2 = \sigma/\mu$. The Waters (מַיִם, *mayim*, 'waters'; Chapter 6) give you field equations. Symmetries (Chapter 7) give you conservation laws. The Five Principles (Chapter 8) constrain the action. The seven pattern operators (Chapter 9) generate all field dynamics.
...
The central insight of this chapter is simple enough to fit on a napkin: **the extra dimensions of the zone manifold have finite extent**. The $\xi$-direction (Waters Above) spans from $\xi = 0$ to $\xi = \xi_A \approx 3 \times 10^{26}$ m. The $\eta$-direction (Waters Below) spans from $\eta = 0$ to $\eta = \eta_B \approx 1.3 \times 10^{-15}$ m. The Firmament membrane wave equation — which we already derived in Chapter 5 — must satisfy boundary conditions at these edges. We do not assert finite extent here for the first time: it was *established* earlier in this volume — the Firmament's membrane structure and finite thickness in Chapter 5 (§5.3) and the exponential decay of the Waters envelopes that bounds the effective $\xi$- and $\eta$-supports in Chapter 6 (§6.3–6.4, where $\Psi_B$ decays exponentially away from the Firmament). This section takes those bounds as given and asks what they imply, which is why the next step is Sturm-Liouville theory rather than a fresh derivation of the boundaries.
...
**Which topology is winding here?** This is worth stating precisely, because Chapter 3 (§3.2.2, Theorem 3.2.3) established that the *base manifold* $\mathcal{M}_Z$ is generically simply connected ($\pi_1(\mathcal{M}_Z)$ trivial in the semi-compact model). There is no contradiction. The winding that quantizes angular momentum is *not* winding of the base manifold; it is winding in the *field/order-parameter configuration*, classified by the homotopy of the vacuum manifold $\pi_1(\mathcal{M}_{\text{vac}}) = \mathbb{Z}\times\mathbb{Z}$ (the vortex winding of §10.3) and, equivalently here, the winding of the phase $e^{im\varphi}$ of the wave function around a defect. A simply-connected base manifold can — and does — support field configurations with non-trivial winding number, exactly as a topologically trivial plane supports vortices in a complex field. The two senses of "topological winding" should not be conflated: the base manifold's $\pi_1$ is trivial (Ch3 §3.2.2); the field/gauge winding is not.
