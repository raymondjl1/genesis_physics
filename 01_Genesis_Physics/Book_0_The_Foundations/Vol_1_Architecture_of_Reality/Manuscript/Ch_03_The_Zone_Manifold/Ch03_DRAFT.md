...
In Chapter 1, we established six axioms and Postulate F that define our universe.[^axiom-count]

[^axiom-count]: Chapter 1 distinguishes the six foundational axioms (Axioms 1–6: Open System, 6D Spacetime, Firmament Mechanics, Sabbath Boundary, Fall Phase Transition, Waters Duality) from Postulate F (the Firmament as codimension-2 Firmament) — Postulate F is a working hypothesis to be verified by the geometry derived in Chapter 3, not an independent axiom at the same foundational level. By the end of this chapter, the zone manifold will be shown to require the Firmament's existence; Postulate F thus becomes a theorem of the axiom set. In Chapter 2, we built the mathematical toolkit—manifolds, curvature, fiber bundles, the language of differential geometry. Now comes the most important step: we translate the axioms into geometry. We build the actual space—the *manifold*—in which physics happens.

Here's the deep truth: **the shape of spacetime encodes the structure of reality itself.** Einstein taught us this. But we're going one layer deeper. The Genesis Physics axiom (Axiom 1.1) says the universe is an *open system*—it is sustained from outside, by a sustaining field. This means spacetime is not self-contained. It has structure that reaches beyond the visible cosmos. It has *zones*.
...
- §3.6 specifies the metric structure and proves consistency with Axiom 1.2 (6D spacetime)
...
From Axiom 1.2 (Chapter 1 §1.2), the spacetime metric is:
...
This is the geometric reason for Axiom 1.3 (Symmetries Imply Conservation Laws): persistent submanifolds define the topological obstructions to changing quantities.
...
1. **The sustaining field κ** (Axiom 1.1, Chapter 1) permeates the cosmos. It is not localized to one zone; it touches all zones.
...
In general relativity (Chapter 1, Axiom 1.3), causality is encoded by light cones. The future light cone of an event $(t, \vec{x})$ on the Firmament is:
...
**Axiom 1.4 (Chapter 1) says:** Consciousness is a fundamental interface between the transcendent and temporal realms.
...
Because it satisfies all six axioms and vindicates Postulate F (Chapter 1):

1. ✓ **Open System:** The zones explicitly encode a source external to the observable cosmos (Heaven Prime → sustaining field).

2. ✓ **6D Spacetime:** The metric (1.3.1) is exactly the form required by Axiom 1.2.

3. ✓ **Symmetries → Conservation:** The bundle structure creates symmetry groups; integration (via Noether's theorem, Vol 2) yields conserved charges.

4. ✓ **Human Interface:** Consciousness is a special section of the bundle, touching multiple zones.

5. ✓ **Degradation:** The second law of thermodynamics emerges from the geometry of the Waters Below (entropy).

6. ✓ **Duality:** The metric separates into 4D (temporal) and 2D (eternal) parts; duality is manifest.

7. ✓ **Four Phases:** The four phases (Creation, Edenic, Fall, Redemption) correspond to different regimes of the sustaining field κ, discussed in Chapter 1 and elaborated in Vol 2.
...
| $\kappa$ | Sustaining field | Axiom 1.1 (Ch 1) |
