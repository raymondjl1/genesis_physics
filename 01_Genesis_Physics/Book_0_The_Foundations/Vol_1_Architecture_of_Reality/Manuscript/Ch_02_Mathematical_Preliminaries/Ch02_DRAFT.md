...
[^idx-ch2]: This index convention is the series-wide canon set by Appendix B §B.1.2. Any text in subsequent chapters or volumes that writes the 6D range as "0 through 5" is a drafting artifact and should be read as the canonical set $\{0,1,2,3,5,6\}$. Latin indices $i, j, k$ run over spatial coordinates (1 through 3). The Einstein summation convention applies unless stated otherwise: repeated upper-lower index pairs are summed. The full 6D embedding space has metric signature $(-,+,+,+,+,+)$: one timelike dimension and five spacelike dimensions. The standard 4D spacetime within the Firmament $Z_{2.2}$ has signature $(-,+,+,+)$. These signatures will be fully motivated in Chapter 4; here, we note them as conventions for any metric computations that follow.
