...
> **Plain-English Summary (for the non-specialist).** The great conservation laws — energy, momentum, electric charge — say certain quantities can never be created or destroyed, only moved around. Why should the universe be so disciplined? A century ago Emmy Noether proved a beautiful answer: every conservation law comes from a *symmetry*, a way the universe stays the same when you shift it. Energy is conserved because the laws of physics are the same today as yesterday; momentum because they are the same here as there. This chapter derives those symmetries from the zone manifold's own geometry, so the conservation laws are not assumed — they are earned. And it adds the deeper "why" standard physics omits: the symmetries themselves reflect an unchanging Maker (an interpretation laid out honestly, not smuggled into the math).
...
[Sign correction Rev. 2026-05-14: The kinetic terms had the wrong sign ($+\frac{1}{2}$) in an earlier version of this equation. The correct sign is $-\frac{1}{2}g^{AB}\partial\Psi\,\partial\Psi$ throughout this series (Lorentzian signature, positive kinetic energy). This is now consistent with Ch 6 Eq. (1.6.4) and the series sign convention established there.]
...
Because God is eternal.[^ax3-energy] "I the LORD do not change" (Malachi 3:6). If the nature of God does not depend on *when* you examine it, then the laws of physics — which reflect that nature through the zone manifold — cannot depend on time either.

[^ax3-energy]: Axiom 3 (Chapter 1, §1.3) establishes the formal connection: the symmetries of the zone manifold correspond to divine attributes, and every continuous symmetry yields a conserved current via Noether's theorem. This section applies that axiom to the specific case of divine Eternality → time-translation symmetry → conservation of energy. In Chapter 4, we identified the Killing vector $K^\mu_{(t)} = \partial_t$ (Eq. (1.4.32)) as the generator of time translations on the zone manifold. The 6D metric (1.4.2), the Firmament action (1.7.3), and the Waters action (1.7.4) are all constructed to be time-translation invariant. Noether's theorem then guarantees a conserved charge. That charge is energy.
...
> **⚠ CORRECTION (Rev. 2026-05-14):** In an FRW background with time-dependent scale factor $a(t)$, the vector $K^A_{(t)} = \delta^A_0$ is **NOT** a Killing vector in general, because $\mathcal{L}_{K_{(t)}} g_{AB} = \partial_t g_{AB} \neq 0$ when $\dot{a} \neq 0$. Energy conservation in expanding zone geometry requires a different approach: use the Killing vector of the static exterior metric, or appeal to the covariant conservation $\nabla_A T^{AB} = 0$ via the zone field equations. The energy conservation theorem stated here holds in the **static limit** ($\dot{a} = 0$, i.e., Minkowski or static extra-dimensional geometry) or **asymptotically** (at late times when expansion is negligible). This section requires revision to give a correct energy conservation proof for the dynamical case; the proper treatment in the full FRW background is deferred to Vol 5.

This generates the transformation $t \to t + \epsilon$, $x^i \to x^i$, $\xi \to \xi$, $\eta \to \eta$. The total action $S_{\text{total}}$ is invariant **in the static approximation** because:
...
- The Firmament tension $\sigma$ and mass density $\mu$ are constants
...
The individual sectors are *not* separately conserved — energy flows between them. The Firmament absorbs energy from the Waters; matter radiates energy into the Waters Below; the Waters Above drive expansion. But the *total* is exactly conserved:
...
An observer on the Firmament does not measure 6D integrals. They measure 4D quantities. By integrating over the extra dimensions (a process we will formalize as Kaluza-Klein reduction in Volume 2, Chapter 1), the 6D conservation law projects to:
...
Because God is equally present everywhere.[^ax3-momentum] "Where can I go from your Spirit? Where can I flee from your presence?" (Psalm 139:7). If no location in space is privileged over any other, then the laws of physics must be the same at all spatial points.

[^ax3-momentum]: Axiom 3 (Chapter 1, §1.3) — applied here to the case of divine Omnipresence → spatial-translation symmetry → conservation of momentum. The same axiom will connect rotational symmetry to conservation of angular momentum in §7.4.2. This is spatial translation invariance. From Chapter 4, the three spatial Killing vectors are:
...
A classical symmetry can be destroyed by quantum effects. The most important example is the **chiral anomaly** (Adler-Bell-Jackiw, 1969).[^anomaly]

[^anomaly]: The anomaly computation requires evaluating a linearly divergent triangle Feynman diagram with two gauge bosons and one axial-current vertex, regulated via Pauli-Villars or dimensional regularization. The full calculation is beyond the scope of Foundations; see Peskin & Schroeder, *An Introduction to Quantum Field Theory*, Chapter 19 (pp. 655–670) for the detailed derivation. The result — that anomaly cancellation in the Standard Model constrains the fermion content — is presented here as a structural constraint; its derivation is deferred to the advanced QFT treatment in Volume 5.
