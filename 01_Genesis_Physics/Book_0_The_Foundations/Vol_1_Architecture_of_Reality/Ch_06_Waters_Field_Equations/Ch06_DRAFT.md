...
In Chapter 5, we established the Firmament as a dynamical membrane — a codimension-2 hypersurface with its own geometry, tension, and vibration spectrum. We showed that the speed of light is the wave speed on this membrane ($c^2 = \sigma/\mu$, Eq. (1.5.0)), that Lorentz invariance follows from the uniformity of the membrane, and that the Firmament couples to the bulk through its extrinsic curvature.

But the Firmament is not alone. It sits between two vast regions of the 6D bulk: the Waters Above ($Z_{2.2.3}$, extending in the $\xi$-direction) and the Waters Below ($Z_{2.2.1}$, extending in the $\eta$-direction). These are not empty space. They are filled with dynamical scalar fields — $\Psi_A$ in the Waters Above, $\Psi_B$ in the Waters Below — that carry energy, exert pressure, and shape the large-scale structure of the cosmos.
...
$$S = S_{\text{grav}} + S_A + S_B + S_{\text{int}} + S_{\text{brane}} \tag{1.6.3}$$
...
- $S_{\text{brane}}$ = Firmament (brane) action (established in Chapter 5)

The gravitational and brane actions were constructed in Chapters 4 and 5 respectively. Here we construct $S_A$, $S_B$, and $S_{\text{int}}$.
...
The Firmament membrane sits at the interface between the two Waters regions. The membrane is in mechanical equilibrium when the pressures balance:

$$P_A\big|_{\xi_0} - \sigma\,K^{(\xi)} = P_B\big|_{\eta_0} - \sigma\,K^{(\eta)} \tag{1.6.42}$$

where $P_A|_{\xi_0}$ and $P_B|_{\eta_0}$ are the Waters pressures evaluated at the Firmament, and $K^{(\xi)}$, $K^{(\eta)}$ are the mean extrinsic curvatures from Chapter 5 (Eqs. (1.5.19)-(1.5.20)). The membrane tension $\sigma$ mediates the balance — it is the "elastic response" of the Firmament to the competing pressures of expansion and contraction.
...
$$\Psi_A\big|_{\xi \to \xi_0^+} = \Psi_A\big|_{\text{brane}} \tag{1.6.43}$$

$$\Psi_B\big|_{\eta \to \eta_0^-} = \Psi_B\big|_{\text{brane}} \tag{1.6.44}$$
...
where $\sigma_{\Psi_A}$ is the coupling of $\Psi_A$ to the brane stress-energy and $M_6$ is the 6D Planck mass. A similar condition holds for $\Psi_B$ in the $\eta$-direction:
...
[FIGURE: Fig 1.6.5 — Boundary Conditions at the Firmament. Vertical cross-section showing the Firmament at $(\xi_0, \eta_0)$. Above: $\Psi_A(\xi)$ approaches $v_A$ asymptotically, with a derivative jump $[\partial_\xi \Psi_A]$ at the brane. Below: $\Psi_B(\eta)$ decays exponentially from its brane value $\Psi_{B,0}$, with derivative jump $[\partial_\eta \Psi_B]$ at the brane. Key labels: continuity conditions at brane, derivative jumps, asymptotic values. The derivative jumps are shown as slope discontinuities in the field profiles.]
...
- $\Omega_\Lambda \propto \xi_A$ (extent of Waters Above region)
- $\Omega_{\text{DM}} \propto \eta_B$ (extent of Waters Below region)
- $\Omega_b \propto \delta$ (Firmament thickness)

The hierarchy $\xi_A \gg \eta_B \gg \delta$ produces $\Omega_\Lambda > \Omega_{\text{DM}} \gg \Omega_b$. The 5% baryonic fraction reflects the fact that baryonic matter IS the membrane — a thin surface between two vast bulk regions. Its smallness is structural, not coincidental.
...
**Problem 6.8.** Verify the pressure balance at the Firmament (Eq. (1.6.42)). Using the values from Symbol_and_Constants.md ($\sigma = 6.0 \times 10^{98}$ kg/(m·s²)), estimate the magnitude of each term and show that the membrane tension dominates.
