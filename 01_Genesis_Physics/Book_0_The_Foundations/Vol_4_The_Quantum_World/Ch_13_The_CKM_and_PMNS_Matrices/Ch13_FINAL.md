...
Before we do any of that, the honesty commitment. Every numerical result in this chapter inherits the fermion-mass error bars of Chapter 10, which range from fifteen percent at the top of the ladder to nearly a factor of ten at the bottom. The structural results — that both matrices are $3\times 3$ unitary, that the Jarlskog invariant of the quark sector is nonzero, that there are exactly two independent neutrino mass-squared differences — do not inherit those error bars. They are rigorous consequences of the framework's topology and nothing else. This chapter's center of gravity is the structural results. The numerical results are reported honestly, with labels, and the places where the framework cannot yet keep up with experiment are stated plainly.
...
In the pattern-operator language of Vol 1 Ch 9, the three generations are the three normalizable eigenstates of the membrane bound-state operator, and "three, no more, no less" is a property of that operator inherited by every downstream construction. The $3\times 3$ unitarity of the mixing matrices is therefore not a feature of the flavor sector specifically; it is a feature of the pattern-operator spectrum propagated to the flavor sector via the Yukawa coupling. This is what we mean when we call Result 13.1 "structural" rather than "flavor-specific."
...
The framework's formula for the CKM matrix element $V_{ij}^{\rm CKM}$ is then
...
These factor-of-2 error bars are not cosmetic; they are traceable. Every Wolfenstein parameter's uncertainty propagates from three specific entries in Chapter 10 §10.9's fermion-mass ledger — the $V_{cb}$-related overlap (inheriting the charm and bottom mass uncertainties, 15-30%), the 1-to-3 cross-generation overlap (inheriting the up-to-top hierarchy error, up to 99% on the up mass), and the imaginary part of the topological overlap phase (which has no independent error bar in Ch 10 at all, because Ch 10 only reported magnitudes). Any tightening at Ch 10 §10.9 propagates directly into §13.3.
...
Second, and this is the key mechanistic difference, the three η-boundary ripple ground states are *nearly degenerate* in mass. The boundary potential is shallow, and the mass splittings between the three states are second-order in the ripple amplitude. Why is the boundary potential shallow? Because the η-boundary is exactly where the Vol 1 Ch 5 warp factor reaches its smallest value — the boundary is the *foot* of the warp, not its peak — and all field-theoretic energies there are suppressed by a corresponding factor. The bulk ξ-ladder lives where the warp factor is larger, so its potential is deep; the η-boundary lives where the warp factor is small, so its potential is shallow. The same warp factor that gives the framework its mass hierarchy between generations (Ch 10 §10.4) gives it the near-degeneracy of the neutrino sector. This contrasts sharply with the ξ-ladder, where the three states are separated by ratios of order fifty or more in mass. The neutrino sector is nearly degenerate; the charged fermion sector is strongly hierarchical. This one contrast will be the whole story of §13.5.
...
where $\psi_i^\nu$ are the three η-boundary ripple eigenfunctions and $V_{\rm bdry}(\eta)$ is the boundary potential (whose leading polynomial form — a shallow quartic with a small quadratic perturbation — is given in `06-NEUTRINO_PHYSICS.md` §4.2). The charged-lepton rotation $U_\ell$ is a ξ-integral exactly like the CKM ones. Their product gives the PMNS matrix elements.
...
This is a live prediction. Unlike the CKM phase — which is already measured precisely — the lepton Dirac phase is *not yet known* to better than about $0.5\pi$, and the next decade will change that. If DUNE finds $\delta_{\rm CP}^\ell\approx 3\pi/2$, the framework's heuristic chirality argument is confirmed. If DUNE finds $\delta_{\rm CP}^\ell\approx 0$ (CP-conserving), the framework's argument is *wrong*, and a specific piece of the ξ-bulk-versus-η-boundary structural story has to be revisited. It is worth being precise about what "revisited" means here: the chirality-asymmetry heuristic lives in `06-NEUTRINO_PHYSICS.md` Part 8, not in the Vol 1 foundations or the Ch 10 ladder construction. A failure of the $3\pi/2$ prediction would invalidate that heuristic and require a more careful calculation of the relative phase between the ξ and η sectors, but it would not break the three-generation count, the $3\times 3$ unitarity of $U_{\rm PMNS}$, or the bi-large mechanism of §13.5 — all of which are structural results that stand independently of what $\delta_{\rm CP}^\ell$ turns out to be.
...
### Chapter Equation Recap

For reference, the eight load-bearing equations of the chapter, collected in one place:

$$V_{\rm CKM} = U_u^\dagger U_d \quad\text{(definition)} \tag{4.13.34}$$

$$U_{\rm PMNS} = U_\ell^\dagger U_\nu \quad\text{(definition)} \tag{4.13.35}$$

$$N_{\rm phases}^{\rm CP}(n\text{ gen.}) = \frac{(n-1)(n-2)}{2}\quad\text{(KM counting, inherited from Ch 11 (4.11.39))} \tag{4.13.36}$$

$$V_{ij}^{\rm CKM}\propto\int d\xi\,\chi_i^{u*}(\xi)\,v(\xi)\,\chi_j^{d}(\xi)\quad\text{(overlap-integral form)} \tag{4.13.37}$$

$$J_{\rm CP}^{\rm quark}\approx A^2\lambda^6\eta\quad\text{(Wolfenstein-leading Jarlskog)} \tag{4.13.38}$$

$$\tan 2\theta=\frac{2|V|}{m_j - m_i}\quad\text{(universal two-state mixing formula; hierarchy/degeneracy lever)} \tag{4.13.39}$$

$$P(\nu_\alpha\to\nu_\beta)=\left|\sum_i U_{\beta i}U_{\alpha i}^* e^{-i m_i^2 L/(2E)}\right|^2\quad\text{(oscillation probability)} \tag{4.13.40}$$

$$\text{Ch 13}:\ J_{\rm CP}^{\rm quark}\;\longrightarrow\;\text{sphaleron rate}\;\longrightarrow\;\text{Vol 5}:\ \eta_B\approx 6\times 10^{-10}\quad\text{(baryogenesis handoff)} \tag{4.13.41}$$

These are the load-bearing eight. The full equation ledger of the chapter runs (4.13.1)–(4.13.41), distributed across §§13.1–13.9 as cataloged in Ch13_OUTLINE.md.
...
**P3 (Computational).** Using the framework's PMNS parameters from (4.13.21) ($\sin^2\theta_{12}^\ell\approx 0.30$, $\sin^2\theta_{23}^\ell\approx 0.50$, $\sin^2\theta_{13}^\ell\approx 0.022$) and the mass splittings from (4.13.23) ($\Delta m_{21}^2\approx 7.5\times 10^{-5}$ eV², $|\Delta m_{32}^2|\approx 2.5\times 10^{-3}$ eV²), compute the three-flavor oscillation probability $P(\nu_\mu\to\nu_e)$ from (4.13.30) at $L/E = 500$ km/GeV (Super-Kamiokande atmospheric) and $L/E = 1{,}300$ km/GeV × $(2.5$ GeV$)^{-1}$ (DUNE baseline). Repeat with $\delta_{\rm CP}^\ell = 0, \pi/2, \pi, 3\pi/2$ and plot the four curves. What fraction of the total oscillation probability is CP-dependent?
