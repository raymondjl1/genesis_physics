...
Chapter 8 confronted the divergences that lurk inside every loop integral and showed that zone architecture resolves them with a physical cutoff: the membrane thickness η_B gives a maximum wavenumber, making every integral finite. But we left a question hanging. The vacuum — the state with no particles at all — is not empty. Chapter 6 showed that the free-field Hamiltonian has the form
...
The **UV scale** is set by the membrane thickness $\eta_B \approx 1.3 \times 10^{-15}$ m (Vol 1 Ch 5). This gives the zone cutoff $\Lambda_{\rm zone} = \hbar c/\eta_B \approx 2.4 \times 10^{19}$ GeV — the highest-energy mode that fits on the Firmament.
...
3. **Firmament tension (mechanical).** The membrane itself has an intrinsic tension $\sigma_{\rm membrane}$ (dimensions: force per unit length, or energy per unit area).
...
Using the zone parameters $\eta_B = 1.3 \times 10^{-15}$ m and $\xi_A = 1.4 \times 10^{26}$ m: (a) Compute $\eta_B/\xi_A$. (b) Compute $(\eta_B/\xi_A)^3$. (c) Multiply $\rho_{\rm vac}^{(\rm QFT)} \approx 2 \times 10^{71}$ GeV⁴ by $(\eta_B/\xi_A)^3$ and compare with $\rho_{\rm vac}^{(\rm obs)} \approx 3.5 \times 10^{-47}$ GeV⁴. How close is the agreement?
