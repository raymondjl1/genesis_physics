...
**Exit condition.** Reader has $v$ in hand and understands the geometric origin of the $\mu^2$ scale (membrane tension divided by cosmological scale squared).
...
1. **What we have.** The form $V_{\rm eff}(H) = -\mu^2|H|^2 + \tfrac{\lambda}{4}|H|^4$ is rigorously the reduced potential. The sign of $\mu^2$ (negative, triggering SSB) is rigorous given the boundary condition $\psi_1(0) = 0$ plus the membrane tension coupling.
...
3. **What is currently fit.** Three $\mathcal{O}(1)$ coefficients: (a) $\beta$ in $\mu^2 = \beta\sigma c^2/\xi_A^2$, from the detailed shape of the membrane-tension coupling near $\xi = 0$; (b) $\alpha$ in the fermion-overlap exponential $y_n = y_0 e^{-\alpha n^2}$ (already gapped in Ch 10); (c) $\lambda_A$, the 6D quartic, which is matched to the measured $m_H$ rather than computed from renormalization group flow.
...
**Conditional note.** This entire derivation requires a primordial spinor field on the membrane (Assumption 10.1 from Ch 10). If OPEN 10.1 is closed differently, the ξ-parity correspondence might require adjustment. Stated openly.
