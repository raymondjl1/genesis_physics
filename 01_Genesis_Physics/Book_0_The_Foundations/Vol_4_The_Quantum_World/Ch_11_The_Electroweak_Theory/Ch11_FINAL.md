...
Here $g$ and $g'$ are the $SU(2)_L$ and $U(1)_Y$ gauge couplings. Their numerical values we will pin down in §11.5; for now they are two of the three input numbers (the third being $v$, which comes from the Higgs mechanism). The $W_\mu^a$ are the three $SU(2)_L$ gauge fields; the $B_\mu$ is the single $U(1)_Y$ gauge field.
...
- The sign of $\mu^2$ is negative, triggering SSB, because the Dirichlet BC $\psi_1(0) = 0$ combined with the sign of the membrane-tension coupling forces the boundary-induced quadratic contribution to be destabilizing.
...
where $J^\pm_\mu$ are the charged weak currents. The standard definition of the Fermi constant identifies this with $(2\sqrt 2 G_F/2) J^+_\mu J^{-\mu}$ (the factor-of-two conventions vary by textbook; I am using the one in which the effective Lagrangian of muon decay has the form $-(G_F/\sqrt 2)[\bar\nu_\mu \gamma^\mu(1-\gamma_5)\mu][\bar e\gamma_\mu(1-\gamma_5)\nu_e]$):
...
The Waters Above field $\Psi_A$ is defined only for $\xi \in [0, \xi_A]$. There is no mirror copy at $\xi \in [-\xi_A, 0]$. Below the membrane is the Waters Below, a structurally different field $\Psi_B$ with its own support on $\xi \in [-\eta_B, 0]$. The Firmament at $\xi = 0$ is not a symmetry plane; it is a boundary.
