...
(i) $\Psi$ is a complex scalar on the 3D observable slice, obtained from the full 6D membrane displacement $\psi_{\text{6D}}$ by integrating along the bounded $(\xi,\eta)$ directions — a projection, not an independent field.
...
The factor of $1/2$ is not a second, independent Gaussian coincidence. It is the *same* half from §3.4.5. The two "halves" are one "half": the Gaussian profile that minimizes the 6D action is precisely the 3D projection of the function that saturates Cauchy–Schwarz, and the coefficient they share is the coefficient of the second moment of a normalized non-negative density.
