...
This chapter demonstrates that entanglement is not "spooky action at a distance" but rather a geometric reality: the zone manifold connects what the 3D projection separates, so what appears nonlocal in 3D is actually local on the higher-dimensional topology. The chapter derives the CHSH Bell inequality bound explicitly from membrane dynamics and compares it with classical limits and experimental results.
...
| Ch4-005 | EXPLICIT CHSH DERIVATION from membrane dynamics: CHSH ≈ 2√2 ≈ 2.828 | V4-QM-DERIVATIONS | NOT MET |
...
| Membrane dynamics and wave equations | Vol 1, Ch 5 |
...
3. **Why is CHSH exactly 2√2 in the framework?** — Because the maximum correlation achievable by membrane excitations that obey the architecture constraints is the geometric maximum set by the zone topology.
...
5. **Why "monogamy" — why can't one particle be entangled with two others equally strongly?** — Because there is a finite total "zone connectivity budget" set by the membrane topology. Strong entanglement with one partner constrains entanglement with others.
...
| 3 | CHSH from membrane excitations | Membrane correlation function C(a,b) | CHSH = 2√2 from zone geometry | (4.4.20)–(4.4.35) [**CRITICAL**] |
...
- **Key content:** 
  ...
  - Correlation function from membrane mode overlap and zone geometry.
...
- **Research source:** 05-QM_FROM_MEMBRANE_DYNAMICS.md § 8.5 provides the core concept (entanglement mediated by perpendicular dimensions). The CHSH derivation must be developed from first principles using the membrane dynamics and zone topology established in earlier chapters.
