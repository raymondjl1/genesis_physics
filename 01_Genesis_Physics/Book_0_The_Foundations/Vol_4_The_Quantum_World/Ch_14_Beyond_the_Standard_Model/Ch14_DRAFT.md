...
*Status:* The Higgs potential written in Ch 11 §11.4 as $V(H) = -\mu^2 |H|^2 + (\lambda/4)|H|^4$ contains two dimensionless parameters, $\mu^2/v_B^2$ and $\lambda$, that are currently fit to precision electroweak data rather than derived. Both should, in principle, come from the boundary conditions on the $\Psi_A$ and $\Psi_B$ fields at the membrane; the derivation has been sketched but not closed. Effort estimate: 3–6 months.
