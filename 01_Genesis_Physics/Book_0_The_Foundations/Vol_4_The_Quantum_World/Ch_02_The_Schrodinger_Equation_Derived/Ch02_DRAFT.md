...
Our target is the single most famous equation of twentieth-century physics: the time-dependent Schrödinger equation. In any other textbook it arrives by fiat. Griffiths writes it down on page one of chapter one and says "where does this come from? It cannot be derived from anything you already know. It came from the mind of Schrödinger." Sakurai introduces it as a postulate, cross-references Dirac, and moves on. Dirac himself, in the opening of *Principles*, warns the reader that the fundamentals of quantum mechanics must be *assumed* because they cannot be derived from classical ideas.
...
2. **Why is there a factor of i?** A real physical membrane displacement is a real number. A complex-valued equation — an equation that literally cannot be solved by real functions — seems to come from nowhere.
...
From Volume 1, Chapter 5, the transverse displacement field of the Firmament brane satisfies
...
- $\sigma = 6.0 \times 10^{98}$ kg/s² — the Firmament's tension, from (1.5.3).
- $\mu = 6.7 \times 10^{81}$ kg/m³ — its surface mass density, from (1.5.4).
- $c = \sqrt{\sigma/\mu} = 3.0 \times 10^{8}$ m/s — the membrane's wave speed, equal (as shown in (1.5.6)) to the speed of light. This is not a coincidence; in zone architecture it is a definition.
...
Here $\eta_{B} \approx 1.3 \times 10^{-15}$ m is the nuclear confining scale, $\xi_{A} \approx 1.4 \times 10^{26}$ m is the Hubble scale, and $\beta_{\text{geom}} \approx 1.16$ is the dimensionless geometric prefactor from the warped 6D metric, all established in Vol 1 Chs 3–5. The numerical value 1.0546 × 10⁻³⁴ J·s agrees with the CODATA value of Planck's reduced constant to within 0.001 %.
...
This motivates the following move. Let the total membrane displacement of a localized defect be written as
...
[FIGURE: Fig 4.2.1 — The Carrier-Envelope Factorization. Two-panel time series at a fixed spatial point. Top panel: the full membrane displacement ψ(t) oscillating at the Compton frequency ω₀ = mc²/ℏ ≈ 7.76 × 10²⁰ rad/s. Bottom panel: the slow envelope |Ψ(t)|, varying on an atomic timescale ∼ 10⁻¹⁵ s. A vertical dashed line marks one carrier period T_c = 2π/ω₀ ≈ 8 × 10⁻²¹ s. The caption reads: "One envelope cycle contains ∼ 10⁵–10⁶ carrier oscillations. Lab measurements see only the envelope."]
...
Before we substitute, one cleanup. The membrane equation (1.5.1) contains the stochastic forcing $\mathcal{F}(x,t)$ from the Waters fields. Its amplitude, from (1.6.*), scales as
...
What remains is the deterministic membrane equation
...
A constant term $C\Psi$ on the right side of (2.4.8) generates a global phase rotation $\Psi \mapsto \Psi\,e^{-i C t/\hbar}$ under evolution, which is unobservable in any experiment that does not compare the envelope with an outside reference clock. We absorb it by choosing the zero of energy to sit at the rest energy — which is the standard, and conceptually obvious, move in non-relativistic physics. (A note on the factor of $1/2$: the full rest energy $m c^2$ is split across two places in the reduction — half of it sits in the constant shift we are about to absorb, and the other half lives inside the carrier $e^{-i E_0 t/\hbar}$ itself, which is why the envelope phase evolves at $e^{-i\epsilon t/\hbar}$ rather than $e^{-i E t/\hbar}$ after the shift. The total is still $E_0$, as it must be; the split is a bookkeeping artifact of the rotating frame.) Define
...
The membrane equation (1.5.1) is *second* order in time. A second-order PDE in time has two independent sets of initial conditions — for the membrane wave equation, one specifies $\psi(x,0)$ and $\partial_{t}\psi(x,0)$. That is two real fields, or equivalently, one complex field.
...
|---|---|---|---|
| Membrane wave equation | (2.3.6) | Inheritance 1 | (1.5.1) |
...
**Q3.** The Schrödinger equation is first-order in time. The membrane equation is second-order. Explain how a second-order PDE can produce a first-order envelope equation without losing any degrees of freedom. Where is the "missing" first-order equation, and what does it describe physically?
...
**X2.** Reinstate the stochastic Waters forcing $\mathcal{F}(x,t)$ in the membrane equation and show that, after the envelope ansatz and averaging over sub-Planck timescales, it contributes a dissipation term to the envelope equation of the schematic form
