...
A note about this chapter's epigraph. Gen 1:14–18 — "*Let there be lights in the firmament of the heaven to divide the day from the night; and let them be for signs, and for seasons, and for days, and years*" — is not chosen as decoration. Read against the physics, it identifies three distinct architectural facts that the present chapter relies on. *First*, the lights are placed *in the firmament* — that is, on the same 4D submanifold $\Sigma \equiv Z_{2.2}$ that Vols 1–2 derived and on which the gauge structure of Ch 11 lives. The flavor degrees of freedom we mix in this chapter are degrees of freedom *of that surface*. *Second*, the lights "divide" — they are differentiating structure; they are not a single object but a discrete set that distinguishes one thing from another. Three generations of quarks and three generations of leptons (Ch 10 §10.3) are exactly the kind of *discrete, distinguishing* structure to which the text refers, and the CKM and PMNS matrices are the language in which one differentiating set (mass eigenstates) is rotated into another (flavor eigenstates). *Third*, the lights are "for signs and for seasons" — that is, they encode time-resolved, observation-bearing patterns. CP-violating phases in $V_{\rm CKM}$ and $U_{\rm PMNS}$ are exactly that: phases that produce observable time-resolved asymmetries — neutral-meson oscillation patterns, neutrino-oscillation $L/E$ patterns — by which one *reads off* the structure of the underlying mixing. The flavor signatures of this chapter are, in the framework's terms, the "signs and seasons" of Gen 1:14: discrete distinguishing patterns on the firmament that carry time-resolved information. That this is the same vocabulary the text uses for the same architectural feature is not coincidence; it is what the text's "firmament" is for.
