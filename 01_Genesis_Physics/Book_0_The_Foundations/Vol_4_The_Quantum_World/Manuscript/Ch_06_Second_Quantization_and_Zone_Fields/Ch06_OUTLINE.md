...
- **"Why" entry point:** In Chapter 5 we saw that "particles" on the Firmament are localized Firmament membrane excitations coupled to the Waters environment. If that is what particles are, then the basic object of the theory is not the wavefunction of an individual particle — it is the displacement field of the Firmament membrane itself.
- **Key content:**
...
- **Key content:**
  ...
  - Intuition pump: on a vibrating membrane, the same physical mode can be ringing with one, two, or ten quanta of energy — and there is no fixed "number of excitations" built into the Firmament itself. The Firmament is always there; the question is how loudly each mode is ringing.
...
- **Key content:**
  - Write down the Firmament wave equation μ∂²ψ/∂t² = σ∇²ψ (from eq. 2.5.4 and the Euler-Lagrange equation derived from it).
...
- **Key content:**
  - Canonical momentum π(x,t) = μ∂_tψ(x,t) from the Firmament Lagrangian via Legendre transform.
  ...
  - Emphasize: the commutators are *derived*, not imposed — they are forced by the Vol 2 Ch 5 Firmament Lagrangian combined with the standard Dirac quantization prescription, and the prescription itself was justified in Ch 1–2.
...
- **Key content:**
  - Define the vacuum |0⟩ by â_k|0⟩ = 0 for all k. Motivate: it is the state with the minimum energy the Firmament can have, the quietest the Firmament membrane can be.
...
- **Topic sentence:** The Firmament Lagrangian of Vol 2 Ch 5, subjected to the mode expansion and second quantization of §6.3, yields a Hamiltonian that is a sum of independent harmonic oscillators — one per mode — and the total energy spectrum is ℏω_k(N̂_k + 1/2).
...
- **Key content:**
  - Write the Hamiltonian density from L_Firm: H = π²/(2μ) + (σ/2)|∇ψ|² + (potential terms).
  ...
  - The Heisenberg equation of motion: iℏ ∂_t ψ̂ = [ψ̂, Ĥ], recovering the classical Firmament wave equation for ⟨ψ̂⟩.
...
- **Key content:**
  ...
  - One-paragraph closing in the Feynman voice: "We have taken the Firmament from one of its corners and given it a quantum life. What remains is to teach it to interact, to count, and — the hardest part — to grow half-integer spin out of its own geometry."
