...
Imagine a seminar room. It is late in the afternoon of the last day of a week-long workshop on the foundations of the Standard Model, and a visiting physicist — call him the Skeptic, because the Skeptic is always in the front row and is always visiting — has just sat through a thirteen-chapter guided tour.

He has seen the Schrödinger equation derived as the slow-envelope limit of a Firmament membrane wave equation (Ch 2). He has seen the uncertainty principle emerge from a 6D embedding (Ch 3). He has watched Bell's inequality violated by a bulk zone correlator rather than by spooky action at a distance (Ch 4). He has worked through renormalization as a zone-boundary regulator (Ch 8), listened politely to the derivation of QED from the firmament Lagrangian (Ch 7), and nodded — or at least not walked out — when the SU(3) × SU(2) × U(1) gauge group fell out of the isometries of a 6D manifold (Ch 11, with the QCD color sector extended in Ch 12). In the final chapter of Part III he saw the CKM and PMNS matrices emerge from overlap integrals on the ξ-ladder and the η-boundary, respectively, with the three-generation count handed down — as a structural, moderate-robustness result — from Ch 10 §10.3.

Now the Skeptic raises his hand. He has one question, and it is the only question that matters for a framework of this kind: *"All right. I grant, for the sake of argument, that the Standard Model is in your framework. Now tell me something the Standard Model does not say — and tell me how I would kill your framework if it were wrong."*

That question is the right question, and the reason it is the right question is worth saying plainly. A framework that cannot say something new is a framework that does not justify its additional machinery — why carry the 6D manifold, the Waters, the ξ-ladder, if all they do is reproduce what Weinberg and Salam already wrote down? And a framework that says something new but not testable is not science; it is metaphysics with equations. The Skeptic is protecting both of those edges at once. This chapter owes him an answer on both.
...
The honesty commitment that Chapter 13 stated at the end of its opening section still holds. Every numerical result in this chapter inherits the fermion-mass error bars of Chapter 10 — which range, at their worst, from fifteen percent to factors of a thousand. Structural results — the existence of exactly one CKM CP phase, the unitarity of PMNS — do not inherit those error bars, and those two this chapter treats as rigorous. The three-generation count is also structural — it does not inherit the mass error bars either — but it carries the weaker label APPROXIMATE (moderate robustness), exactly as Ch 10 §10.3 labels it: robust over a factor-of-two variation in the well parameters, not a protected topological invariant. "Structural" and "rigorous" are not the same claim, and this chapter does not conflate them. Numerical results are reported with their labels. Open problems are reported as open. There is no place in this chapter where a gap is hidden behind a softening word. The one-thousand-fold mass-scale problem is named as "the one-thousand-fold mass-scale problem" — not as "an outstanding precision question" or "a residual issue." The spin-$1/2$ blocker is named as a blocker. The CP phase is reported at its current order-of-magnitude precision and no better. This is not modesty; it is the minimum the framework can afford if it wants the Skeptic to come back tomorrow.

The chapter's structure is laid out in the roadmap figure below (Fig 4.14.1): a brief callback to the hierarchy problem of Vol 2 Ch 9, the four-class dark sector, the collider-prediction set, and the two ledgers — the falsification table and the research roadmap — that hand forward to Volumes 5 and 6.
...
The key equation, inherited from Vol 2 Ch 9 §9.4 as the scale-ratio relation, takes the form
...
This section is a recap. The reader who wants the full derivation should return to Vol 2 Ch 9 §9.4. What this chapter wants to do is move on — to the sectors of the framework the Standard Model does not contain at all.
...
Four structurally distinct candidate classes emerge. The framework does not promote any single one as *the* answer — it offers all four as possibilities, each with its own mass scale, its own coupling regime, and its own experimental signature. A genuine dark-matter discovery could, in principle, select among them. These four classes exhaust the sectors of the current 6D Lagrangian that do not directly couple to the Chapter 10–11 charged-current vertices; any fifth class would require extending the architecture beyond its present form, and the absence of a fifth class is part of the framework's testable content.

**Class A — Heavy KK excitations in the η-direction.** The Waters Below are bounded in η by a potential wall at $\eta = \eta_B$, and the transverse modes of that potential form a Kaluza–Klein tower. Chapter 6 built the tower as the Dirichlet-mode spectrum of the η-confined wave equation, yielding masses of the form

$$M_{m,\eta} = \hbar c \cdot \frac{m\pi}{|\eta_B|}, \qquad m = 1, 2, 3, \ldots \tag{4.14.2}$$

With $|\eta_B| \approx 1.3 \times 10^{-15}$ m, the first KK mode sits at $M_{1,\eta} \approx 0.95$ GeV, the second at $\approx 1.90$ GeV, the third at $\approx 2.85$ GeV, and so on up to the Firmament cutoff.

The stability of any such KK mode as a dark-matter candidate requires a selection rule that forbids decay into lighter all-Standard-Model final states. In the zone architecture, such a rule arises from a discrete gauge symmetry inherited from the η-orbifold of Chapter 12. The η-direction, as Ch 12 §12.3 showed, is constructed as a $\mathbb{Z}_3$-identified interval whose endpoint structure gives the $SU(3)$ color group; the residual discrete symmetry at the boundary — call it $\mathbb{Z}_2^{\rm KK}$, because it acts as a parity on even versus odd KK modes — is not broken by any Standard Model vertex, and it protects a specific linear combination of even-$m$ modes against decay into SM particles. This is the same mechanism that gives lightest-KK-particle stability in the Appelquist–Cheng–Dobrescu universal-extra-dimension construction, translated into the zone-QFT formalism.

The resulting candidate is the lightest $\mathbb{Z}_2^{\rm KK}$-protected mode, which in the current analysis sits at approximately 0.95–1.9 GeV, with a nucleon-scattering cross-section driven by the same η-boundary vertex that mediates Z-boson couplings but suppressed by one loop factor ($\sim \alpha_W/4\pi \approx 2 \times 10^{-3}$). Order-of-magnitude,

$$\sigma_{A}^{\rm nucleon} \sim 10^{-44} \text{ to } 10^{-42} \text{ cm}^2. \tag{4.14.3}$$

The two-decade range is not stylistic — it is an honest reflection of the fact that the η-boundary vertex structure has not yet been computed at precision; that calculation sits in the research roadmap as **RR-7** (running couplings) and **RR-8** (neutrino mass generation, which shares the η-boundary machinery), and tightening this range is one of the concrete payoffs of closing those items. Class A is, with those caveats, the Genesis framework's best collider-accessible dark-matter candidate and its most directly falsifiable. If SuperCDMS or the next-generation directional detectors reach a nucleon-recoil cross-section of $10^{-46}$ cm$^2$ at the 1–2 GeV mass window and find nothing, Class A is ruled out.
...
with the exact value set by a seesaw between the η-boundary ripple and a residual Majorana contribution from the Waters Above — both of which are currently labeled as open problem **RR-8** in §14.6. Couplings to ordinary neutrinos are suppressed by the same overlap integrals that give the PMNS mixing angles; a typical mixing with active neutrinos is of order $|U_{\alpha s}|^2 \sim 10^{-3}$ to $10^{-4}$. Direct-detection cross-sections are cosmologically irrelevant (well below $10^{-50}$ cm$^2$), but short-baseline reactor experiments such as PROSPECT, STEREO, and their successors can constrain the mixing directly.
...
**Class D — Radion / dilaton modes.** The distance between the two Waters — the ξ-modulus — can oscillate. Its quantum is a scalar field, the radion, whose mass is set by whatever potential stabilizes the ξ-modulus. In the framework's current analysis, moduli stabilization is open problem **RR-9** in §14.6, which is why the radion mass is not yet predicted from first principles. A typical stabilization mechanism based on Casimir forces between the two Waters (Vol 4 Ch 9) gives a modulus oscillation frequency set by the Casimir energy density $\sim \hbar c / \xi_A^4$, which at $\xi_A \sim 3 \times 10^{26}$ m translates to an energy quantum of order a milli–electron-volt. The resulting radion mass is in the range

$$m_{\rm rad} \sim 10^{-3} \text{ to } 10^{-1} \text{ eV}, \tag{4.14.7}$$

with a gravitational-strength coupling to all Standard Model fields. We should be candid about what (4.14.7) is and is not: a range spanning two orders of magnitude is a prediction only in the loosest sense — it cannot be cleanly falsified, because almost any null result in a given decade of mass can be accommodated by retreating to the other decade. We therefore label the radion mass **PHENOMENOLOGICAL** rather than APPROXIMATE: the order-of-magnitude estimate follows from the Casimir-energy scale, but the factor that would collapse the range to a single testable value is exactly what the open moduli-stabilization problem (RR-9) has not yet supplied. Radions in this mass range are classified as *ultralight* dark matter and have distinctive signatures in atomic-clock experiments, resonant-mass detectors, and pulsar-timing arrays. The Genesis framework's Class D prediction overlaps with the search windows of current and proposed ultralight-dark-matter experiments, including atom-interferometric programs and resonant-mass detector programs — but until RR-9 closes, that overlap is a target of opportunity, not a sharp prediction.
...
We state this as a formal result, with its structural and its approximate components separated.

**Result 14.1.**

**(a) Structural, RIGOROUS.** *Within the current Lagrangian of Chapters 10 through 13, no state acquires a WIMP-like nucleon coupling at the electroweak scale. The dark sectors of Genesis Physics are exhausted by the four classes above: heavy KK modes (Class A), η-boundary sterile neutrinos (Class B), ξ-continuum WIMPless states (Class C), and the radion modulus (Class D). No additional class can be introduced without modifying one of Chapters 10 through 13.*

**(b) Approximate, falsifiable.** *The candidate mass ranges and cross-sections reported in equations (4.14.2)–(4.14.7) are inherited from the currently uncomputed η-boundary vertex structure (RR-7, RR-8) and the ξ-modulus stabilization potential (RR-9). Closing those open problems will sharpen the numbers. The no-WIMP statement in part (a) is independent of those numbers and survives any sharpening of them.*

The structural half of Result 14.1 is the framework's sharpest dark-sector prediction: if a canonical WIMP is discovered, the framework is in trouble not because of a miscounted loop factor but because its Lagrangian does not contain a sector that produces one. The approximate half is what the experimentalist can pin down within the current decade.

[Fig 4.14.2: Dark-matter candidate space. A log-log plot of candidate mass ($x$-axis, from $10^{-6}$ eV to $10^5$ GeV) against nucleon-recoil cross-section ($y$-axis, from $10^{-60}$ cm$^2$ to $10^{-38}$ cm$^2$). The four Genesis classes are marked as ellipses: Class A (labeled "A") at 0.95–1.9 GeV, $\sim 10^{-44}$–$10^{-42}$ cm$^2$; Class B (B) at $10^{-2}$–$1$ eV, $\sim 10^{-50}$ cm$^2$; Class C (C) at 10–100 GeV, $\sim 10^{-60}$ cm$^2$; Class D (D) at 1–100 meV, no direct nucleon coupling (marked with a special symbol). The current XENONnT and LZ exclusion curve is drawn as a solid line. The canonical WIMP window (100 GeV, $10^{-45}$ cm$^2$) is shown as an empty box with a line through it and the label "no Genesis candidate."]
...
$$\rho_{\rm vac}^{\rm loop} \sim \frac{v_A^4}{(4\pi)^2}\,, \tag{4.14.10}$$

where the $O(1)$ prefactor depends on which fields run in the loop and can differ by one or two orders of magnitude depending on the field content. Taking (4.14.10) at face value gives $\rho_{\rm vac}^{\rm loop} \sim 10^{62}$ GeV$^4$, which is approximately 184 orders of magnitude larger than the observed dark-energy density,
...
What the framework *does* offer, tentatively, is a mechanism for the cancellation. The $\Psi_A$ and $\Psi_B$ fields are related by a $\mathbb{Z}_2$ symmetry in the Vol 1 Ch 3 construction — the Waters Above and Waters Below are two reflections of a single underlying structure. Under that $\mathbb{Z}_2$, the sign of the vacuum energy contribution from each Waters sector is opposite, and the *net* vacuum energy contains a leading cancellation between the two. In the limit of an exactly unbroken $\mathbb{Z}_2$, the cancellation would be perfect and the vacuum energy would vanish identically; in the framework, the $\mathbb{Z}_2$ is softly broken by the Firmament boundary conditions (the Waters are *not* identical — the Waters Below supports quarks and gluons while the Waters Above does not), and the residual is the net vacuum energy. How large is the residual? The framework does not yet know. Vol 5 will return to this; the problem is listed in §14.6 as **RR-12** (among low-priority items, not because it is unimportant — it is the single largest quantitative problem in theoretical physics — but because closing it requires tools the Vol 5 cosmological sector has not yet developed).
...
Chapters 10 and 11 made four quantitative predictions for the masses of the heavy bosons of the Standard Model, each derived from the ξ-ladder overlap integrals and the electroweak symmetry breaking of the Waters Below, plus the $\rho$-parameter constraint that ties them together:
...
The prediction, stated precisely: the current central values of $m_t$, $m_H$, $M_W$, $M_Z$, and the $\rho$ parameter are preserved under further precision measurements to within their current PDG precisions, which at the time of writing are roughly 0.5% or better for the four boson masses and 0.1% for the $\rho$ parameter. The framework tolerates deviations at those precisions; it does not tolerate deviations that break the simultaneous agreement.

The falsification threshold is set by the joint structure of the derivation. The boson-sector quantities are not independent — they are all computed from the two inputs $g_W$ and $v_B$, so a shift in any one of them implies a shift in the others by correlated amounts. The sharpest test is therefore a *joint* condition:

> **Falsification (Prediction 14.1).** *The framework is falsified if (i) any single boson-mass observable shifts from its current PDG central value by more than two percent (corresponding to roughly four times the current best experimental precision, or two sigma above the derivation's own geometric tolerance), OR (ii) any two boson-mass observables shift by more than one percent each in the same direction (which would indicate a coherent shift in $g_W$ or $v_B$ inconsistent with the Ch 11 §11.5 derivation), OR (iii) the $\rho$ parameter deviates by more than 0.5%.*

The 2%/1%/0.5% thresholds are not arbitrary. They are set so that a confirmed shift at those levels cannot be absorbed into the $O(1)$ uncertainties in the $g_W$ or $v_B$ derivation of Ch 11 without forcing one of the prior chapters to be rewritten. Below those thresholds, the framework's derivation is compatible with experimental precision; above them, it is not.
...
The reason this prediction has teeth is not that any individual boson mass is special. It is that *four masses plus the $\rho$ parameter* must all simultaneously agree with a framework that has, in the boson sector, exactly one dimensionless geometric input ($g_W$) and one dimensional input ($v_B$). Everything else is a derived quantity. A framework with that much input cannot wiggle out of a shift in any single observable by retuning; a shift anywhere is a shift everywhere, and a coherent shift above the joint thresholds breaks the whole boson sector at once.

### Prediction 14.2 — Exactly three generations

Chapter 10 §10.3 showed that the ξ-ladder bound-state spectrum on the Vol 1 Ch 5 double-well potential has exactly three normalizable eigenstates. Three, not four, not two. This is the eigenvalue count of that particular double-well — three normalizable bound states for a well of that depth. The count is *robust but not bulletproof*, exactly as §10.3 labels it: it is stable under a factor-of-two variation in the well parameters ($V_0$, $\eta_B$), but the double-well form itself is the leading-order approximation to the bulk-geometry potential, and higher-order corrections could in principle shift the count. The count does **not** depend on the Higgs profile, on the Yukawa overlap integrals, or on any of the downstream Ch 10 error bars — those affect the *masses*, not the *number* of states; it depends only on the shape of the double-well potential, which is set by the Vol 1 Ch 5 zone geometry. That is the precise sense in which the prediction is structural rather than a consequence of mass-fitting, and the precise sense in which it is nonetheless only approximate: it inherits the moderate robustness of the §10.3 spectral count, not the bulletproof status of a topological invariant.
...
**Result 14.3 (APPROXIMATE — structural, moderate robustness).** *Genesis Physics predicts exactly three fermion generations. The generation count is the normalizable-bound-state count of the Vol 1 Ch 5 double-well spectrum (§10.3): it is independent of the Higgs profile, the Yukawa overlaps, and all downstream mass error bars, but it inherits the moderate robustness of the §10.3 count — stable over a factor-of-two variation in the well parameters, not protected by a topological invariant. Discovery of a fourth-generation fermion at any mass falsifies the framework.*

This is the strongest *structural* prediction in Volume 4 — structural because it depends on the shape of the core spectral problem rather than on any downstream mass calculation. It is labeled APPROXIMATE rather than RIGOROUS to match §10.3 exactly: the count is robust over a factor-of-two window in $V_0$ and $\eta_B$ but is not a bulletproof topological invariant, because the double-well is itself the leading-order form of the bulk potential, and the $\mathbb{Z}_2$ well-degeneracy question (OP-03) is still open. The falsification claim is unaffected by this label — a fourth generation kills the framework regardless of how robust the count's *derivation* is. It remains the sharpest knife in §14.5's falsification table.

### Prediction 14.3 — The Higgs branching-ratio pattern

The Higgs boson, in the framework's construction (Ch 11 §11.7), couples to Standard Model fermions through the same overlap integrals that give the fermion masses. Its decay branching ratios are therefore direct predictions — no new parameters, no new inputs. Chapter 11 §11.7 recorded the seven dominant channels (PDG values rounded; the charm channel in particular is currently known only to coarse precision):

| Channel | BR (Genesis) | BR (PDG, 2024) |
|---|---|---|
| $h \to b\bar b$ | $58.0\%$ | $58.4 \pm 0.7\%$ |
| $h \to W^+ W^-$ | $21.5\%$ | $21.4 \pm 0.8\%$ |
| $h \to gg$ | $8.6\%$ | $8.2 \pm 0.4\%$ |
| $h \to \tau^+ \tau^-$ | $6.3\%$ | $6.3 \pm 0.4\%$ |
| $h \to c\bar c$ | $2.9\%$ | not yet precisely measured |
| $h \to ZZ$ | $2.6\%$ | $2.6 \pm 0.1\%$ |
| $h \to \gamma\gamma$ | $0.23\%$ | $0.227 \pm 0.010\%$ |

The framework's prediction is that *no new decay mode appears above 0.5% that is not in this list*. In particular, $h \to \text{invisible}$ — decay into any dark-sector state of §14.2 — is predicted to be below 2%. Current ATLAS and CMS bounds on the invisible fraction are at approximately 11% (95% CL), so the framework's 2% prediction is not yet tight but will be tightened at HL-LHC. If the invisible fraction is measured to exceed 5% with 3σ significance, a new decay channel beyond the framework's four dark-matter classes has been opened, and the framework must either identify which of its classes is coupling more strongly than expected or acknowledge that a fifth class exists.
...
**Falsification threshold:** any new decay mode above 0.5% branching ratio not in the seven-channel list, or any listed channel deviating from the Genesis prediction by more than 3σ relative to the experimental measurement.
...
Flavor-changing neutral currents are the Standard Model's cleanest probe of physics beyond it. In the SM, FCNCs — processes like $t \to c\gamma$, $t \to cZ$, $b \to s\gamma$, $K^0 \to \mu^+\mu^-$ — arise only at loop level, and they are suppressed by the GIM mechanism, which in turn depends on the unitarity of the CKM matrix.
...
and at next-to-leading order (NLO) as approximately $\Gamma_t^{\rm NLO} \approx 1.56$ GeV. The PDG value is $\Gamma_t^{\rm obs} = 2.00 \pm 0.1$ GeV. The gap between prediction and observation is approximately 25 percent.
...
But this is an argument, not a calculation. The framework owes the calculation. The Genesis prediction, therefore, is a prediction *about the outcome of the calculation*: when it is performed, the result will shift from 1.56 GeV to approximately 2.0 GeV, closing the 25% gap to within a few percent. The fact that the calculation has not yet been done does not make this non-predictive, any more than a theorist's prediction about the outcome of a not-yet-completed experimental run is non-predictive. The falsification criterion is sharp: if the NNLO calculation yields a value below 1.85 GeV or above 2.15 GeV — that is, fails to close the gap to within the current experimental error bar — then the framework's top-sector vertex structure is wrong, and the derivation of Ch 11 §11.8 must be revisited.

This is an internal falsification test. It is listed as open problem **RR-6** in §14.6, with an effort estimate of 3–4 months and a HIGH priority label.

**Falsification threshold:** NNLO zone-QFT calculation of $\Gamma_t$ gives a value below 1.85 GeV or above 2.15 GeV.

**Experiment:** internal calculation, roughly two-year timescale.
...
[Fig 4.14.4: The boson-mass precision ledger. Five vertical bars, one each for $m_t$, $m_H$, $M_W$, $M_Z$, and $\rho$ (each normalized to its PDG central value). Each bar shows the Genesis prediction as a solid point, the current PDG value as a hollow point (indistinguishable from the Genesis point at this resolution), and a shaded 2% band above and below the PDG value as the per-observable falsification envelope. A dashed line at 1% indicates the joint-shift condition. Caption notes: "all five predictions currently lie within their error bars; the framework's claim is that the bars remain simultaneously satisfied as precision improves, and no coherent shift above the joint threshold appears."]
...
|---|---|---|---|---|---|---|
| 1 | $W$ boson mass | $80.27$ GeV | $80.377 \pm 0.015$ GeV | $>2\%$ deviation (or joint $>1\%$ with other boson) | CMS/ATLAS $W$-mass | now–2030 |
| 2 | $Z$ boson mass | $91.55$ GeV | $91.188 \pm 0.002$ GeV | $>2\%$ deviation (or joint $>1\%$) | LEP/LHC precision | now |
| 3 | Higgs mass | $125.1$ GeV | $125.10 \pm 0.14$ GeV | $>2\%$ deviation (or joint $>1\%$) | HL-LHC | 2024–2030 |
| 4 | Top quark mass | $173.0$ GeV | $173.1 \pm 0.4$ GeV | $>2\%$ deviation (or joint $>1\%$) | HL-LHC | 2024–2030 |
...
| 8 | $h \to $ invisible BR | $< 2\%$ | $< 11\%$ (95% CL) | $> 5\%$ measured | HL-LHC | 2024–2035 |
...
| 14 | Top quark decay width (NNLO) | $\sim 2.0$ GeV (gap must close) | $2.00 \pm 0.1$ GeV | NNLO calc yields $<1.85$ or $>2.15$ GeV | internal calculation | ~2 years |

Fourteen rows, and three things to notice about them.

The sharpest knife in the table is row 6. A fourth-generation particle, at any mass, at any collider, kills the framework in a single datum. No other row gives the framework this kind of immediate vulnerability, and no other row is as clean — no error bar, no joint condition, no required calculation. Either a fourth generation is discovered or it is not, and the discovery ends the argument.

The largest existing success in the table is rows 1 through 5 taken together: the boson-mass precision envelope, with the $\rho$ parameter as the tie that binds them. Five simultaneous agreements, all within current experimental precision, from a framework that has *two* inputs in the boson sector. This is the piece of numerical evidence on which the framework currently rests its claim to seriousness, and it is the piece that the next decade of HL-LHC precision work will either confirm or demolish.

The largest existing discomfort is row 14: the top-quark decay-width gap. The framework has not yet calculated itself out of a 25 percent discrepancy between prediction and measurement, and its defense of the gap — "NNLO corrections will close it" — is an argument the framework has not yet checked. The Skeptic is right to point at row 14 and ask the framework to close it; the framework owes him the calculation, and RR-6 in the next section is the note-to-self that says so.

There is a fourth thing to notice, and it is not a row — it is a shared spine running underneath several of them. The nuclear scale $\eta_B \approx 1.3 \times 10^{-15}$ m is treated throughout Volume 4 as a *fixed input*, not a derived quantity, and the framework does not yet predict why $\eta_B$ has the value it does or why that value "lands" the electroweak scale to the right order of magnitude (the roughly sixteen-order-of-magnitude coincidence flagged in Ch 11 §11.3 and Ch 12 §12.3, and routed to moduli stabilization RR-9 in §14.6). The consequence for falsification is worth stating bluntly: $\eta_B$ is not independently adjustable per chapter. Any future determination that forced $\eta_B$ to a materially different value would not falsify one row of this table — it would propagate through every chapter that uses it (the KK tower of Ch 6, the confinement scale of Ch 12, the dark-sector mass ranges of §14.2, and the boson-mass envelope of rows 1–5 through its role in the warp factor), and would have to be reconciled simultaneously across all of them. That shared dependency is a vulnerability and an honest one; it is the reason RR-9 is on the roadmap rather than buried.

Knife, success, discomfort, shared spine. Those, plus the rest of the table, are the experimental stance Volume 4 hands to Volume 6.
...
**RR-3. Higgs potential $O(1)$ coefficients** (GitHub issue #25). The Higgs potential written in Ch 11 §11.4 as $V(H) = -\mu^2 |H|^2 + (\lambda/4)|H|^4$ contains two dimensionless parameters, $\mu^2/v_B^2$ and $\lambda$, that are currently fit to precision electroweak data rather than derived. Both should, in principle, come from the boundary conditions on the $\Psi_A$ and $\Psi_B$ fields at the Firmament; the derivation has been sketched but not closed. Effort estimate: 3–6 months. Impact: Ch 11 §11.4 becomes a full derivation of the Higgs mass (not just the ratio $m_H/v_B$), and the boson-mass precision envelope of Prediction 14.1 gains predictive power at the sub-percent level.

**RR-4. CKM CP phase $\delta_{\rm CP}^q$** (GitHub issue #3). Ch 13 §13.3 reported the Jarlskog invariant at order of magnitude only: $J_{\rm CP}^{\rm quark,\,framework} \sim$ few $\times 10^{-5}$. The PDG value is $(3.18 \pm 0.15) \times 10^{-5}$. The framework's current prediction has an uncertainty of about one decade, because the CP phase depends on the complex phases of the ξ-ladder overlap integrals, which Ch 10 §10.4 did not compute — it only reported the magnitudes of the overlaps, not their phases. Resolution: extend Ch 10 §10.4 to compute the overlap-integral *phases*, which requires a careful treatment of the topological structure of the ξ-ladder bound-state wavefunctions. Effort estimate: 3–4 months. Impact: Ch 13 §13.3 upgrades its Jarlskog invariant from APPROXIMATE (order-of-magnitude) to APPROXIMATE (within a factor of 2). This also tightens the baryogenesis discussion of RR-12.

**RR-5. Grand unification.** The framework has three gauge groups — $SU(3)_C$ from the η-orbifold (Ch 12), $SU(2)_L$ from the Waters Below isometry (Ch 11), $U(1)_Y$ from the Firmament hypercharge (Ch 11) — but it does not yet tell us whether they unify at high energy, and if so, into what. Vol 2 Ch 10's running coupling analysis put the three couplings within a few percent of a common value at approximately $10^{16}$ GeV, which is suggestive of a GUT but does not prove one. The framework has not yet committed to a specific GUT structure: $SU(5)$, $SO(10)$, and an $SU(4) \times SU(2) \times SU(2)$ Pati–Salam variant are all in principle compatible with the zone architecture, and a zone-specific group (not on the conventional GUT list) is also possible. Resolution: compute the geometric unification condition from the 6D curvature integral, which should pick out one GUT structure from the candidates. Effort estimate: 4–6 months. Impact: the framework predicts the proton decay rate from the GUT structure, which becomes a sharp test against Hyper-K and future proton-decay experiments. This also closes the question of whether $v_A \approx 10^{16}$ GeV is *literally* the GUT scale or merely adjacent to it.

**RR-6. NLO/NNLO corrections in zone QFT.** The framework's predictions are mostly tree-level. Ch 8 developed renormalization at one loop. Higher orders have not been done. The top-decay-width gap of Prediction 14.5 is the sharpest sign that higher-order corrections matter; the boson-mass envelope of Prediction 14.1 will also tighten once NNLO corrections are computed. Effort estimate: 3–4 months. Impact: Prediction 14.5 is resolved (either the gap closes and the framework is vindicated, or it does not and the framework's top-sector vertex is wrong). Prediction 14.1 gains sub-percent precision. The RG analysis needed for RR-2 Path C becomes possible.
...
**RR-8. Neutrino mass generation.** Ch 13 §13.4 built the PMNS matrix from η-boundary ripples but did not derive the absolute neutrino masses. The seesaw mechanism needs grounding in the Waters Below structure. Effort: 2–3 months. This is also the calculation that fixes the Class B sterile-neutrino mass range of §14.2.
...
**14.1 (Conceptual).** The framework's prediction of exactly three fermion generations (Result 14.3) is labeled APPROXIMATE — structural, moderate robustness. What is the structural input on which the count depends, and why does that input make the prediction *structural* (independent of the Yukawa/mass calculations) yet only *moderately robust* (not a protected topological invariant)? If the Vol 1 Ch 5 double-well potential were replaced with a triple-well potential, how many generations would the framework predict? What aspect of the zone geometry *prevents* this replacement from being made arbitrarily, and which open problem (OP-03) would have to be closed to upgrade the label from APPROXIMATE to RIGOROUS?
...
**14.4 (Conceptual).** Prediction 14.1 (the boson-mass precision envelope) would be trivially satisfied if the framework had only to match one mass — any framework with a single free parameter can be adjusted to match any single observable. Explain precisely why the *simultaneous* matching of $m_t$, $m_H$, $M_W$, $M_Z$, and the $\rho$ parameter to 0.5% or better is a non-trivial constraint on the framework, and not simply an accident of numerical precision. Include in your answer a discussion of why the *joint*-shift condition in the falsification statement (two masses shifting by more than 1% in the same direction) is a sharper test than the per-observable 2% condition.

**14.5 (Calculation).** For dark-matter Class A (the KK mode at 0.95–1.9 GeV), take equation (4.14.3) as your starting point and estimate the integrated exposure (in kg·day) at which SuperCDMS would be expected to either detect the mode or rule out the upper end of the predicted cross-section range. Assume a xenon or germanium target with standard nuclear-recoil detection efficiency.
...
**14.7 (Order of magnitude).** From $v_A \sim 10^{16}$ GeV and the loop-level vacuum-energy estimate (4.14.10), reproduce the 184-order-of-magnitude cosmological-constant mismatch. Then, assuming the $\mathbb{Z}_2$ symmetry of §14.3 provides an exact cancellation in the unbroken limit, estimate the minimum suppression that the *softly broken* $\mathbb{Z}_2$ must supply in order to reach the observed $\Lambda_{\rm obs} \sim 10^{-122}$ GeV$^4$. To how many orders of magnitude must the cancellation be exact? Comment on whether such precision is plausible from a symmetry broken only by membrane boundary conditions.
