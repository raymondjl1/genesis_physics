...
The first is to name the framework's **new predictions** — the places where the zone architecture says something the Standard Model does not, with a number attached. Dark matter candidates from the Waters Below (dark matter, ~27%) with specific masses and couplings. Dark energy from the Waters Above (dark energy, ~68%) with a specific equation of state. Collider signatures: a boson-mass precision envelope, a fixed generation count, a Higgs decay pattern, a top-decay-width discrepancy. These are the places where the framework pokes its head above the Standard Model and says, *look here*.
...
One last thing the Skeptic should be told, because it is not a row but a shared spine running underneath several of them. The nuclear scale $\eta_B \approx 1.3 \times 10^{-15}$ m is treated throughout Volume 4 as a *fixed input*, not a derived quantity, and the framework does not yet predict why it has its value or why that value "lands" the electroweak scale to the right order of magnitude (the roughly sixteen-order-of-magnitude coincidence flagged in Ch 11 §11.3 and Ch 12 §12.3, routed to moduli stabilization RR-9 below). The consequence for falsification: $\eta_B$ is not independently adjustable per chapter. Any future determination forcing $\eta_B$ to a materially different value would not falsify a single row — it would propagate through every chapter that uses it (the KK tower of Ch 6, the confinement scale of Ch 12, the dark-sector mass ranges of §14.2, and the boson-mass envelope of rows 1–5 through the warp factor) and would have to be reconciled across all of them at once.
...
**Predictions §14.2–§14.4 as architectural tests of Gen 1:6–8.** Before the handoff, it is worth saying explicitly what the predictions of this chapter are tests *of*. They are not free-floating phenomenological guesses; they are tests of the same architectural commitment we have been making since Vol 1 Ch 3 — that the cosmos has the structure Gen 1:6–8 names: a tensioned dividing surface (the Firmament, $\Sigma \equiv Z_{2.2}$) with two bulk regions on either side (the Waters Above, carrying $\Psi_A$; the Waters Below, carrying $\Psi_B$). Each prediction class in this chapter falls out of that architecture if and only if the architecture is correct. The four dark-matter candidate classes of §14.2 are excitations of the *Waters* (Classes A and C of the Waters Below, Class B and the sterile sector at the Firmament–Waters boundary, Class D of the inter-Waters modulus); the prediction that no WIMP sits at the electroweak scale is the prediction that *dark matter does not live on the Firmament*, which is what Gen 1:6–8's separation of waters from waters requires. The Waters-Above vacuum of §14.3 with $w = -1$ is the prediction that the *upper* of the two bulks carries the dark-energy condensate; if a future survey measured $w \neq -1$ at high significance, the architectural picture in which the Waters Above is a single condensate would have to be revised. The collider predictions of §14.4 — three generations and no fourth, the Higgs decay-channel ledger, the absence of low-scale supersymmetry, the radion-mass band, the $W$-mass shift — are tests of *what lives on the Firmament*. In each case the prediction is a "yes, this is what is on the dividing surface" or "no, this would not fit." Read together, §14.2–§14.4 are not fourteen separate tests; they are fourteen probes of the same two-bulks-and-a-dividing-membrane architecture that Gen 1:6–8 names. The framework's experimental program is, in the end, a program of testing that architecture.

Volume 5 inherits the cosmological sector of this chapter. The four dark-matter candidate classes of §14.2 become the dark-matter *cosmology* of Volume 5: their abundance calculations, their freeze-out histories, their structure-formation signatures, and their constraints from the CMB acoustic peaks. The Waters Above vacuum of §14.3 becomes the cosmological-constant chapter of Volume 5 — in particular, the proposed Z$_2$ cancellation mechanism named briefly in §14.3 is taken up properly in Vol 5 Ch 8 (and Ch 11). The hierarchy recap of §14.1 becomes the moduli-stabilization problem of Volume 5, intertwined with RR-9 and RR-2 Path D.
