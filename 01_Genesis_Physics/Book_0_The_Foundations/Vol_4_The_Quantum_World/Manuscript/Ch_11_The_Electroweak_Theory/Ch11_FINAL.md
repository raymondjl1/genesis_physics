...
And yet the story is true. The question this chapter has to answer is not whether the zone-architecture framework can reproduce it — the numbers below will show that it can — but what that reproduction costs in honest parameters. So: what does it cost?
...
A note on why the gauge-boson sector is so much cleaner than the fermion sector of Chapter 10. In Chapter 10, I was trying to reproduce *twelve* Yukawa couplings from *one* exponential profile. In Chapter 11, I am trying to reproduce *four* electroweak observables ($M_W, M_Z, m_h, G_F$) from *three* proximate inputs ($g, g', v$). That is why Table 4.11.1 looks so good while Chapter 10's Table 4.10.2 was more of a mixed bag. But I must be careful not to undercount, because $v$ is not itself a free input on the same footing as $g$ and $g'$: as §11.4 will show, the framework's *derivation* of $v$ from cosmological scales currently carries three further $\mathcal{O}(1)$ fit coefficients ($\beta$, $\alpha$, $\lambda_A$; OPEN 11.1, GitHub #25). So the honest end-to-end parameter count for this chapter is not three but closer to six — $g$, $g'$, and the three $\mathcal{O}(1)$ coefficients behind $v$, with $g$ and $g'$ themselves traced to the gauge sector. The four observables are reproduced from those inputs, not conjured from nothing. Parameter-counting is the bookkeeping of honesty.
