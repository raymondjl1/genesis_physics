...
The plan of the chapter is shown as a flow in the roadmap figure below (Fig 4.7.1): the four tools are built in sequence — interaction picture, Dyson series, Wick's theorem, Feynman rules — and then exercised on three calculations of increasing difficulty (tree-level Coulomb scattering, the one-loop vertex correction, and the two precision observables $g-2$ and the Lamb shift), before the chapter closes by confronting the UV divergences with the Firmament cutoff.

[FIGURE: Fig 4.7.1 — Chapter 7 roadmap (flow diagram). A left-to-right pipeline in two bands. Top band, "Tools": four boxes in sequence — "Interaction picture (§7.2)" → "Dyson series & S-matrix (§7.3)" → "Wick's theorem (§7.4)" → "Firmament propagator $D_F$ (§7.5)" → "Feynman rules (§7.6, → Appendix C)". An upstream box "Interaction Hamiltonian from Vol 2 Ch 5 Lagrangian (§7.1)" feeds the first. Bottom band, "Calculations": three boxes fed by the Feynman-rules box — "Tree-level Coulomb scattering (§7.7)" → "One-loop vertex correction (§7.8)" → "Precision observables: $g-2$ (§7.9), Lamb shift (§7.10)". A final box "UV divergences & Firmament cutoff (§7.11)" gathers the loop calculations, with an arrow to "Summary & forward pointer (§7.12)". A dashed tag on the fermion lines reads "Dirac structure used as placeholder until Ch 10."]
