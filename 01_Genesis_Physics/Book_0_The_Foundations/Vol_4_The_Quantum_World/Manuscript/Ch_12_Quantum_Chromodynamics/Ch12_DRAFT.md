...
> **Cross-reference — Z₃ orbifold:** The $\mathbb{Z}_3$ orbifold of $S^1_\eta$ first appears in **Vol 2 Ch 4 §4.2** (zone manifold geometry) and is used in **Vol 2 Ch 6** (gauge group derivation from KK reduction). The result $SU(3)_c$ from $S^1_\eta/\mathbb{Z}_3$ topology is the strongest rigorous structural result of this chapter — it is a direct topological theorem requiring no free parameters. The $\mathbb{Z}_3$ identification is also directly linked to the three-generation count of Ch 10 §10.3 (the Hopf fibration argument giving n_w = 3), which provides independent evidence that the three-fold structure is physically fundamental. See also the APS index theorem argument in the session notes (OP-02 RESOLVED, 2026-05-14).
...
**(Assumption 10.1 — OP-1 / GitHub #1)** The quark field $q = (q_r, q_g, q_b)^T$ in (4.12.18) is a Dirac spinor with anticommuting components. This anticommuting structure has not been derived from the bosonic zone membrane; it is assumed under Assumption 10.1 (Ch10 §10.5). The $SU(3)_c$ color structure and the form of the covariant derivative are rigorous; the fermionic spinor character is the placeholder.
