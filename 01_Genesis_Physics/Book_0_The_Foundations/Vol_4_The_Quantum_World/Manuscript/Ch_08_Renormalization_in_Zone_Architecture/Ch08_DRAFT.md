...
status: SUPERSEDED (was DRAFT synced from FINAL 2026-04-08; superseded 2026-05-16)
superseded_by: Ch08_FINAL.md
created: 2026-04-08
...
> ⚠️ **DEPRECATED — DO NOT USE FOR PRODUCTION** (2026-05-16)
>
> This DRAFT file is superseded by **`Ch08_FINAL.md`** in this directory. Ch08_FINAL.md contains all CT-4.Λ corrections (Λ_zone = 0.152 GeV throughout, corrected from the erroneous Planck-scale value in this file; 9 surgical corrections applied 2026-05-15). If you are working on this chapter, open Ch08_FINAL.md instead.
...
This is close to the conventional Planck scale (~1.22 × 10¹⁹ GeV) but not identical. The difference is because η_B is a geometric parameter of the zone structure, not derived from fundamental constants (ℏ, c, G). In standard physics, the Planck energy is $E_P = \sqrt{\hbar c^5 / G}$, which includes Newton's constant G. In Genesis Physics, the Firmament thickness is independent of G—it comes from the 6D structure of the Firmament itself.

> ⚠ **NUMERICAL ERROR — CT-4.Λ (Rev. 2026-05-14):** The computation in (4.8.11) above contains an arithmetic error. The **correct** evaluation of Λ_zone = ℏc/η_B with η_B = 1.3×10⁻¹⁵ m is:
> 
> ℏc = (1.055×10⁻³⁴ J·s)(3×10⁸ m/s) = 3.165×10⁻²⁶ J·m
> 
> Λ_zone = 3.165×10⁻²⁶ J·m / 1.3×10⁻¹⁵ m = 2.435×10⁻¹¹ J ≈ **0.15 GeV** (proton mass scale)
> 
> The value 2.4×10¹⁹ GeV stated above is wrong by a factor of ~1.6×10²⁰. If η_B = 1.3 fm (nuclear confinement scale), then Λ_zone is at the proton/QCD scale (~150 MeV), **not** the Planck scale. This is a conceptually important distinction: the Firmament thickness in Genesis Physics corresponds to the nuclear scale, not the Planck scale, and the UV cutoff of the theory is therefore far below the Planck energy.
> 
> **Consequence:** All subsequent calculations in this chapter that use Λ_zone ≈ 2.4×10¹⁹ GeV — including the Problem Set in §8.12, Problem 8.5 — inherit this error and require re-evaluation once the correct η_B interpretation is established. The question of whether η_B truly corresponds to the nuclear scale or to some other geometric length is **Open Problem CT-4.Λ**, which must be resolved before the renormalization chapter can be closed. See also Ch09 §9.1 which uses this value for the Casimir calculation.
> 
> The structural argument of this chapter (that the Firmament thickness provides a physical UV cutoff) remains correct. Only the numerical identification of the cutoff scale is in error here.
...
> **Derivation Status — Beta Function Coefficient**
>
> The coefficient $-1/(3\pi)$ in Eq. (4.8.20) is identical to the standard QED result. In conventional QFT, this coefficient is derived by evaluating the one-loop vacuum polarization diagram using dimensional regularization or a momentum cutoff. The zone-architecture version of this derivation — showing that the Kaluza-Klein zero-mode structure of the Firmament reproduces the same coefficient as the standard 4D calculation — is the work of GitHub issue #26 and is in preparation.
>
> What is established here: (1) The functional form of the beta function (logarithmic running) is a structural consequence of the 2D extra-dimensional geometry (a logarithmic Green's function on a 2D compact space gives logarithmic running in 4D — see §8.9). (2) The coefficient must agree with the standard QED result at leading order, because the zone-architecture Lagrangian reproduces the QED vertex at low energies (established in Ch 7). (3) The numerical agreement between the one-loop running (1/137 → 1/129 at $m_Z$) and experiment is within the expected precision of a one-loop calculation — the 1% discrepancy reflects two-loop corrections, not a zone-architecture error. The full in-chapter derivation of the coefficient awaits the two-loop frontier (Open Problem 8.1).
...
These are *finite*, not infinite. They are physical—the values you would measure if you had an experiment at the Planck scale. Above the Firmament scale, there are no more modes, no more virtual loops to compute. The theory is complete.

This solves an age-old critique of QFT: the bare parameters have no operational meaning. In Genesis Physics, they do. They are simply the values the couplings take at the fundamental scale—the Firmament thickness.
...
- **Zone view:** An EFT is valid below the physical membrane scale Λ_zone = ℏc/η_B ≈ 10^19 GeV. The theory is complete below that scale because that is where the zone structure itself ends. Above that scale, you cannot have a 4D quantum field on the Firmament—you are in the realm of the 6D Waters Above/Below.
...
When you try to create a virtual loop at scale k > Λ_zone, the corresponding wavelength λ = 2π/k becomes shorter than η_B. At that point, the virtual particle does not fit on the Firmament. Describing what happens requires understanding the full 6D geometry of the Waters Above and Below—a task beyond 4D QFT.
...
3. **The physical cutoff** (§8.3) in zone architecture is $\Lambda_{\rm zone} = \hbar c / \eta_B \approx 10^{19}$ GeV, derived from the Firmament thickness.
