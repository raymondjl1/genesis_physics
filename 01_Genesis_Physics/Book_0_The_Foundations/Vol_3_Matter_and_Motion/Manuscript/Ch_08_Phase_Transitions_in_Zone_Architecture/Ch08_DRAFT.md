...
In Chapter 7, we derived a remarkable result: the electroweak symmetry of the universe broke spontaneously when the Waters Above (dark energy, ~68%; paired with Waters Below = dark matter, ~27%) scalar field rolled off the top of a Mexican hat potential and into the valley. The W and Z bosons acquired mass. Fermions acquired mass through their overlap with the condensate. The massless, featureless plasma of the early universe became the structured, massive, differentiated cosmos we observe.
...
[FIGURE: Fig 3.8.1 — Chapter Derivation Roadmap (flowchart). **Title bar:** "How Matter Changes State: Phase Transitions from the Membrane." **Top row (inputs, shaded grey to mark prior results):** "Partition function and entropy (Vol 1 Ch 11)" → "Firmament membrane mechanics, tension $\sigma$ (Vol 1 Ch 5)" → "Order parameters / symmetry breaking (Ch 6–7)". **Middle row (this chapter's new derivations, white):** Box A "Van der Waals equation from membrane gauge-field fluctuations" (§8.1) → Box B "Clausius–Clapeyron from chemical-potential equilibrium" (§8.2) → Box C "Landau theory: order parameters and the free-energy expansion" (§8.3) → Box D "Critical phenomena, universality, and the renormalization group" (§8.4) → Box E "Zone-architecture constraints on which transitions are realized" (§8.5) → Box F "The electroweak transition as cosmological bridge" (§8.6). **Bottom row (output, gold):** "A unified phase-transition framework on the zone manifold" → "Bridge to Vol 5 (cosmological phase transitions: recombination, the Sabbath Boundary)." **Arrows:** all forward, with a back-arrow from Box D to Box C showing the "why universality?" feedback loop (critical exponents constrain the Landau expansion) and from Box E to Box C (topology constrains the available order parameters). **Key labels:** order parameter $\phi$, free energy $F(\phi,T)$, critical temperature $T_c$, latent heat $L$, critical exponents $\alpha,\beta,\gamma,\delta$, correlation length $\xi$. **Complexity:** moderate. **Why it's needed:** the chapter spans benchtop thermodynamics to cosmological symmetry breaking; a one-glance map shows readers that the same membrane formalism governs both, and answers "why these transitions and not others?"]
...
This result is exact for *non-interacting* particles. But particles DO interact — Chapter 7 derived the entire mass spectrum from the coupling between vortex defects and the gauge fields of the Firmament membrane. Those same gauge field fluctuations produce forces between particles at finite separation. An equation of state that ignores these interactions cannot describe phase transitions, because phase transitions are precisely what happens when interactions compete with thermal motion.
...
**1. Dispersion forces (attractive, long-range).** The quantum fluctuations of the electron cloud in molecule A generate instantaneous dipole moments. These dipole fluctuations couple to molecule B through the gauge field propagator on the Firmament. The resulting correlation produces an attractive potential that scales as $r^{-6}$:

$$U_{\text{attract}}(r) = -\frac{C_6}{r^6} \quad (3.8.2)$$

where $C_6$ depends on the polarizability of the molecules — itself determined by the wavefunction overlap integrals of Chapter 7. The $r^{-6}$ scaling follows from the structure of dipole-dipole interactions: the instantaneous dipole field falls as $r^{-3}$, and the induced dipole responds linearly, giving $r^{-3} \times r^{-3} = r^{-6}$. More precisely, this is the standard quantum-electrodynamic derivation of the London dispersion force — a *second-order* perturbation effect in which two charge distributions exchange virtual photons — inherited here by the zone framework rather than newly derived; the relevant propagator is the Firmament electromagnetic propagator (cf. Vol 2, Eq. 2.3.18). The $r^{-6}$ law is thus a consequence of standard QED applied on the Firmament, not a result read directly off the photon Green's function.
...
Here $\mu^2 = \sigma c^4 / (2\xi_A^2)$ is the negative mass-squared from Firmament tension (Ch 7, Eq. 3.7.12), and $c = (2m_W^2 + m_Z^2 + 2m_t^2)/(16v^2)$ is a coefficient determined by the particles that couple to the Higgs.
