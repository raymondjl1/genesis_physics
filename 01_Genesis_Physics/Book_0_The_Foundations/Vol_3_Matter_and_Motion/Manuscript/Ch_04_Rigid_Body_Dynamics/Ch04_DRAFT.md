...
for any vector $\mathbf{A}$. The proof is short: express $\mathbf{A} = A_i(t)\hat{e}_i(t)$ in the rotating basis and differentiate using the product rule. The only nontrivial step is $d\hat{e}_i/dt = \boldsymbol{\omega} \times \hat{e}_i$, and here is why it holds. The basis vectors $\hat{e}_i$ are fixed *in* the body, so their only motion is the body's rigid rotation; rigidity means their lengths and mutual angles never change, so each $\hat{e}_i$ can only be turned, never stretched. An infinitesimal rigid rotation by angle $d\theta$ about the instantaneous axis $\hat{\boldsymbol{\omega}}$ displaces the tip of $\hat{e}_i$ by $d\theta\,(\hat{\boldsymbol{\omega}} \times \hat{e}_i)$; dividing by $dt$ and writing $\boldsymbol{\omega} = \dot\theta\,\hat{\boldsymbol{\omega}}$ gives $d\hat{e}_i/dt = \boldsymbol{\omega} \times \hat{e}_i$. In other words, the time derivative of a body-fixed unit vector is exactly the rotation operator $\boldsymbol{\omega}\times(\cdot)$ applied to it. The first term of the product rule, $\dot{A}_i\hat{e}_i$, is then the body-frame derivative; the second, $A_i(\boldsymbol{\omega}\times\hat{e}_i) = \boldsymbol{\omega}\times\mathbf{A}$, is the frame-rotation contribution.
...
The observed "Chandler wobble" has a period of about 433 days — a 127-day discrepancy from the rigid-body prediction (41% error).

> **Note:** The predicted Chandler wobble period differs from observation at the ~41% level. This discrepancy is attributed to the rigid-body approximation: Earth's actual response includes viscoelastic coupling to zone architecture that is not captured in this chapter's treatment. Including Earth's elasticity (Munk & MacDonald 1960; Dahlen 1976) brings the theoretical period to ~435 days, consistent with observation. This is an expected limitation of the classical approximation, not a falsification. The continuum mechanics framework of Chapter 5 provides the foundation for extending this analysis to viscoelastic bodies.
