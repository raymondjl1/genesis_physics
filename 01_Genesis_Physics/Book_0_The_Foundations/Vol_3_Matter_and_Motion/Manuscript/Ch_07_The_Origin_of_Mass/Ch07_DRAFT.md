...
This chapter fills that gap by deriving the mass-generation mechanism from the zone architecture, step by step: the Higgs field emerges as the lowest Kaluza-Klein mode of the Waters Above (§7.1); its Mexican-hat potential is forced by the Firmament boundary conditions (§7.2); spontaneous symmetry breaking gives the W and Z their masses while leaving the photon massless (§7.3); fermion masses arise as overlap integrals between topological vortex defects and the Higgs condensate, with different generations having different overlap (§7.4); and the resulting spectrum is compared against experiment across twelve orders of magnitude (§7.5). The section headings below carry this structure; each section also states honestly where the derivation is complete and where it is calibrated or still open (in particular, the reduction of the electroweak VEV $v = 246.22$ GeV to pure zone-geometry ratios is incomplete — see §7.1 and the Parameter Disclosure in §7.3).

The result is that mass is not a free parameter. Mass is architecture.

[FIGURE: Fig 3.7.1 — Chapter Derivation Roadmap (flowchart). **Title bar:** "Where Mass Comes From: Architecture, Not a Free Parameter." **Top row (inputs, shaded grey to mark prior results):** "6D Waters Above action (Vol 1 Ch 6)" → "Kaluza–Klein decomposition + Firmament boundary conditions (Vol 1 Ch 4–5)" → "Topological vortex defects from Ch 6 (the fermions to be weighed)". **Middle row (this chapter's new derivations, white):** Box A "Higgs doublet as lowest KK mode of the Waters Above" (§7.1) → Box B "Mexican-hat potential forced by Firmament tension" (§7.2) → Box C "Spontaneous symmetry breaking: $W,Z$ get mass, photon stays massless" (§7.3) → Box D "Fermion masses as vortex–Higgs overlap integrals; generations from differing overlap" (§7.4) → Box E "The mass spectrum vs. experiment across twelve orders of magnitude" (§7.5). **Bottom row (output, gold):** "Mass is architecture — the particle spectrum from zone geometry" → "Bridge to Ch 8 (phase transitions) and Vol 5 (cosmological masses)." **Arrows:** all forward; Box C and Box D both feed Box E (a converging fork). One back-arrow from Box E to Box B marks the honesty loop: the electroweak VEV $v = 246.22$ GeV is *calibrated*, not yet reduced to pure geometry (Parameter Disclosure, §7.3). **Key labels:** Higgs field $\phi$, VEV $v$, potential parameters $\mu^2,\lambda$, gauge masses $m_W,m_Z$, Yukawa overlap $y_f$, generation index. **Complexity:** complex. **Why it's needed:** the chapter routes through two parallel mechanisms (gauge masses and fermion masses) that re-converge, and explicitly flags a calibrated input; a one-glance map keeps the open piece visible.]
...
2. **The hierarchy parameter $\alpha \approx 1.0$ is falsifiable.** Under this chapter's convention — higher mode number $n_\xi$ means a more rapidly oscillating vortex profile, smaller overlap with the Higgs, and therefore a *weaker* Yukawa coupling and a *lighter* particle (§7.4, Eq. 3.7.41) — a fourth-generation ($n_\xi = 4$) fermion would be *more* suppressed than the third generation, not less. Its mass would satisfy $m_4 / m_3 \sim e^{-\alpha(4^2 - 3^2)} = e^{-7\alpha} \approx 9 \times 10^{-4}$ for $\alpha \approx 1.0$, placing it *far below* the third-generation scale (sub-MeV for a charged lepton). The corresponding Yukawa coupling $y_4 = y_0 e^{-16\alpha}$ is so small that such a particle would couple far too weakly to the Higgs to have been produced at current colliders, so its non-observation is exactly what the framework predicts (cf. §7.4). The falsifiable content runs the other way: a heavy fourth-generation fermion *above* the electroweak scale — the naive Kaluza-Klein expectation that higher modes are heavier — would *contradict* this chapter's convention and falsify the overlap-suppression mechanism.
