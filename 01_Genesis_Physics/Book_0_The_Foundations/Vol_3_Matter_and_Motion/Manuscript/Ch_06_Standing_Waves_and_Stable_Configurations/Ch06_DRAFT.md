...
[FIGURE: Fig 3.6.1 — Chapter Derivation Roadmap (flowchart). **Title bar:** "From the Vibrating Firmament to Particles That Persist." **Top row (inputs, shaded grey to mark prior results):** "Firmament wave equation + zone-manifold geometry (Vol 1 Ch 5)" → "Extra-dimensional boundary conditions / compactification (Vol 1 Ch 4)" → "Mode quantization on a compact domain (Vol 1 Ch 10)". **Middle row (this chapter's new derivations, white):** Box A "Standing waves on the membrane: from drumheads to the Firmament" (§6.1) → Box B "Mode quantization → discrete particle spectrum" (§6.2) → Box C "The vacuum manifold and symmetry breaking" (§6.3) → Box D "Topological-defect classification by homotopy groups $\pi_n(\mathcal{M})$" (§6.4) → Box E "Fermions from vortices: the Jackiw–Rossi mechanism" (§6.5) → Box F "Topological stability: why matter cannot smoothly decay" (§6.6) → Box G "Pattern operators and the Genesis 'gathering'" (§6.7). **Bottom row (output, gold):** "Matter = topologically protected standing waves of the Firmament" → "Bridge to Ch 7, where these configurations acquire calculable masses." **Arrows:** all forward, with a back-arrow from Box F to Box B and from Box D to Box C showing the "why are masses discrete and stable?" feedback loop. **Key labels:** mode index $n$, wavenumber $k_n$, vacuum manifold $\mathcal{M}$, homotopy groups $\pi_0,\pi_1,\pi_2,\pi_3$, winding number, Chladni-node analogy. **Complexity:** complex. **Why it's needed:** the chapter is the conceptual turning point of the volume — where "stuff" first appears — and braids analogy, geometry, and topology, so readers need a one-glance map.]
