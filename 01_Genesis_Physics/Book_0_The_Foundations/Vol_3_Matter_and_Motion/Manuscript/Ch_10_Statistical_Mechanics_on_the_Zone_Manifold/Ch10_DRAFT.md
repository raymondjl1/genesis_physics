...
[FIGURE: Fig 3.10.1 — Chapter Derivation Roadmap (flowchart). **Title bar:** "Statistical Mechanics: The Counting Machine on the Zone Manifold." **Top row (inputs, shaded grey to mark prior results):** "6D action → quantized modes; $\hbar$, $k_B$, spin-statistics (Vol 1 Ch 10)" → "Partition function $Z$ and the four laws (Ch 9)" → "Membrane geometry of the extra dimensions (Vol 1 Ch 4–5)". **Middle row (this chapter's new derivations, white):** Box A "The partition function on the zone manifold" (§10.1) → Box B "The three statistical ensembles (microcanonical, canonical, grand-canonical)" (§10.2) → Box C "Mode density $g(\nu)$ from membrane geometry" (§10.3) → Box D "The Planck distribution from zone quantization $\times$ the Bose factor" (§10.4) → Box E "Thermal-radiation laws: Stefan–Boltzmann and Wien" (§10.5) → Box F "The classical–quantum bridge" (§10.6). **Bottom row (output, gold):** "Thermal radiation derived end-to-end; the CMB spectrum predicted" → "Bridge to Vol 4 (quantum statistics) and Vol 5 (CMB)." **Arrows:** all forward, with a back-arrow from Box D to Box C showing the "why this mode density?" feedback loop (the Planck fit pins down $g(\nu)$). **Key labels:** partition function $Z$, ensembles, mode density $g(\nu)$, Bose occupation factor, temperature $T$, spectral radiance $B_\nu(T)$. **Complexity:** complex. **Why it's needed:** the chapter assembles a long chain from abstract counting to the observed CMB; a one-glance map shows readers the single thread linking ensembles to a measured spectrum.]
