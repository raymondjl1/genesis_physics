...
| Firmament as codimension-2 brane at $(\xi_0, \eta_0)$ | Vol 1, Ch 5 |
...
| Fig 3.1.2 | Geodesic vs. Forced Motion on the Firmament | Diagram | §1.3, after First Law derivation | Two test particles on a curved brane surface: one in free fall (following geodesic), one under external force (deviating from geodesic). The deviation IS the acceleration. | Makes the geometric meaning of "force" viscerally clear — force is deviation from geodesic, not something mysterious | Firmament surface, geodesic path, forced path, deviation vector $a^\mu$, force vector $f^\mu$ | (2.2.44), (3.1.X) | Medium |
