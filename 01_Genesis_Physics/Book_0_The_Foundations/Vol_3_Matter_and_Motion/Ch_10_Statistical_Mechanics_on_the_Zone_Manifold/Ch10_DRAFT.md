...
3. **Mode density from geometry** (§10.4) — How many electromagnetic modes fit in a box? The answer is pure geometry: g(ν) = 8πν²/c³. We derive it by counting standing waves, just as we counted membrane modes in Vol 1 Chapter 5.
...
3. The entropy $S = -k_B \sum_n p_n \ln p_n$ is **maximized**
...
**Why this matters for Genesis Physics.** In standard treatments, the Boltzmann distribution is often motivated by "contact with a heat bath" — a physical argument. Our derivation is stronger: it is a mathematical theorem about information-theoretic entropy on a discrete state space. The zone manifold provides the discrete states (quantized membrane modes from Vol 1 Ch 10); the maximum entropy principle provides the distribution. No additional physical assumptions are needed.
...
This follows from $F = U - TS$ combined with the identification $S = -k_B \sum_n p_n \ln p_n$ and $U = \sum_n p_n E_n$. The proof:

$$F = U - TS = \sum_n p_n E_n + k_BT \sum_n p_n \ln p_n \tag{3.10.6}$$
...
**Entropy:**
$$S = -\frac{\partial F}{\partial T}\bigg|_{V,N} = k_B \ln Z + \frac{U}{T} \tag{3.10.9}$$
...
**Worked Example: The Two-Level System.** To see the machinery in action, consider the simplest possible system: a single membrane mode with two states (ground state E₀ = 0, excited state E₁ = ε).
...
For a system of quantized membrane modes, the microstates are specified by occupation numbers {n_k} for each mode k. The total energy is:
...
**Entropy:**
$$S(U, V, N) = k_B \ln \Omega(U, V, N) \tag{3.10.18}$$
...
where c = √(σ/μ) is the speed of light derived from membrane tension and mass density (Vol 1 Ch 5). Converting:
...
- **c** (speed of light) ← √(σ/μ) from membrane mechanics (Vol 1 Ch 5)
...
**Numerical verification.** Using the membrane-derived constants:
- k_B = 1.381 × 10⁻²³ J/K
- h = 6.626 × 10⁻³⁴ J·s
- c = 2.998 × 10⁸ m/s
...
**Problem 10.1.** Verify the Stefan-Boltzmann constant numerically. Starting from the membrane-derived values of k_B, h, and c, compute σ_SB and compare with the measured value. Show each step of the calculation.
