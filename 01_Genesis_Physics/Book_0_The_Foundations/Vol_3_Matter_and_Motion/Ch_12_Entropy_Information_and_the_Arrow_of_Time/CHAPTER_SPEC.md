...
| 3 | Landauer's principle from zone architecture | Bit erasure on quantized membrane modes | E_erase ≥ k_B T ln 2 | (3.12.15)–(3.12.18) |
...
| Fig 3.12.3 | Landauer's Principle: The Thermodynamic Cost of Forgetting | Schematic | §12.3, after Eq. 3.12.18 | A two-state membrane mode (bit) being erased: initial state (two possible configurations) → erasure operation → final state (one configuration). Heat flow k_BT ln 2 shown leaving into environment. | Connects abstract information theory to physical reality on the membrane | E₀, E₁, k_BT ln 2, heat arrow | (3.12.15)–(3.12.18) | Simple |
