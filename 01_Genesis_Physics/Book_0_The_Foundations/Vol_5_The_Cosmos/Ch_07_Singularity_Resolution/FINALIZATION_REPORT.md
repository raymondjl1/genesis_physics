...
| 6 | But-Why Item 4 | §7.8.3 (end) | Added a paragraph clarifying what "derived rather than postulated" means in the membrane framework: namely, that $\sigma > 0$ is itself derived from the brane action of Vol 1 §5.6, not assumed independently. The paragraph also notes the substantive difference from the competing programs — they postulate their regularization mechanism *for the purpose of* regularizing, whereas the membrane framework's regularization is a consequence of an action written down for reasons independent of singularities. |
...
| R5.7.4 | Big Bang treated explicitly; brane-nucleation interpretation; forward link to Ch 8 | MET (§7.5, Theorem 5.7.3) |
...
| G2: Cauchy-horizon mass-inflation timescale not computed in brane mechanics | LOW | OPEN — flagged in §7.6.4 and §7.9.3 (now also flagged inline as patch #5) |
...
- Proves the brane–bulk geodesic continuation lemma constructively.
...
- Compares the membrane resolution honestly with LQG, string theory, and asymptotic safety.
