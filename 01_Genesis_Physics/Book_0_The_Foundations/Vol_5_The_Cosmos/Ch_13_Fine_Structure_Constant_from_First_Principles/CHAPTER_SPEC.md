...
| R5.13.3 | Evaluate each input from first principles: (a) ξ_A ≈ 3×10²⁶ m from Vol 1 Ch 6 Waters Above boundary condition (Hubble-scale IR cutoff); (b) η_B ≈ 1.3×10⁻¹⁵ m from Vol 1 Ch 5 inner brane thickness / confinement scale; (c) b_eff ≈ 9.05 from Standard Model particle content (Vol 4 Ch 10) + warp-factor corrections; (d) UV boundary condition α⁻¹(μ_UV) ≈ 0 from the quasi-fixed-point analysis at the inner brane. | 10-FINE_STRUCTURE_DERIVATION.md §§2–5; Vol 1 Ch 5–6; Vol 4 Ch 10 | §4 |
...
- **Vol 1 Ch 5** — Firmament as codimension-2 brane; inner brane thickness η_B ≈ 1.3×10⁻¹⁵ m set by the confinement scale
...
- **Vol 2 Ch 3** — Electromagnetism from membrane wave propagation; §3.7 previewed α⁻¹ ≈ C · ln(ξ_A/η_B); this chapter COMPLETES that derivation
...
- **Vol 4 Ch 10** — Leptons and quarks from membrane resonances (provides the SM particle content that b_eff counts)
...
4. **Why is the UV boundary condition α⁻¹ ≈ 0 instead of ≈ 137?** Because at the inner brane the gauge field is strongly coupled (there is no length scale larger than the brane thickness to run against). In the limit ξ_A → η_B the framework must predict α⁻¹ → 0; the observed α⁻¹ ≈ 137 is a measure of *how much running* there has been between the two boundaries. This flips the usual question on its head: it is not "why is α = 1/137?" but "why is the universe 41 orders of magnitude wider than a proton?"
...
| D4 | Closure via UV boundary condition | Topological structure of inner brane (strong coupling as ξ → η_B); Vol 1 Ch 5 | α⁻¹(μ_UV) ≈ 0 with conservative window [0, 5] | §4, Eqs (5.13.29)–(5.13.31) |
...
| Fig 5.13.3 | Zero-mode gauge field in the warped extra dimensions | §3.2, after solving the 2D EOM | Cross-section schematic | A 2D (ξ, η) plane. ξ runs horizontally from η_B (left) to ξ_A (right); η runs vertically inside the Waters Below stack. Colour shading shows |f₀(ξ, η)|² ∝ ξ^{−2α_f} e^{-γη}: concentrated near the inner brane on the ξ-axis, exponentially suppressed into Waters Below. A dashed line marks the Firmament at η = 0. | Shows why the ξ integral gives a logarithm (power-law times warp → 1/ξ integrand) and why the η integral gives a constant (exponential suppression, nearly all weight at the brane). | ξ, η, η_B, ξ_A, f₀(ξ, η), Firmament, Waters Above, Waters Below | (5.13.13), (5.13.14), (5.13.20) | Medium |
| Fig 5.13.4 | Running of α⁻¹ from the UV brane to the IR horizon | §3.4, after identifying KK with RG | Plot (semilog x) | Horizontal axis: ln μ, from ln μ_UV ≈ ln(10¹⁵ GeV) on the left to ln μ_IR ≈ ln(10⁻⁶ eV) on the right (span 95 natural-log units). Vertical axis: α⁻¹(μ). A straight line of slope −b_eff/(2π) ≈ −1.44 starting from α⁻¹(μ_UV) ≈ 0 at the UV brane and arriving at α⁻¹(μ_IR) ≈ 137.17 at the IR horizon. Mark experimental value 137.036 as a horizontal tick. | Visually the master formula *is* this line. The chapter's central claim — that α is the total running between the two brane scales — becomes a single picture. | α⁻¹(μ), μ_UV, μ_IR, slope = b_eff/(2π), 137.17 (theory), 137.036 (experiment) | (5.13.28), (5.13.32), (5.13.40) | Medium |
