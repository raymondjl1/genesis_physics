...
| R5.8.3 | Each fluid component identified with its bulk-field origin: $\rho_A = \Lambda_A$ from Waters Above potential (Vol 1 Eq 1.6.36), $\rho_B \propto a^{-3}$ from Waters Below VEV (Vol 1 Eq 1.6.40), $\rho_b \propto a^{-3}$ from brane matter, $\rho_r \propto a^{-4}$ from brane radiation | Chapter prompt; 08-FRIEDMANN_EVOLUTION §§2.2-2.3 | §8.5 |
| R5.8.4 | Critical density $\rho_\text{crit} = 3H_0^2/(8\pi G_4) \approx 9.47\times10^{-27}$ kg/m³ derived and physically interpreted as the closure threshold of the brane-projected Waters energy budget | 08-CRITICAL_DENSITY_CALCULATION; chapter prompt | §8.6 |
| R5.8.5 | Density parameter decomposition $\Omega_A = 0.684,\ \Omega_B = 0.266,\ \Omega_b = 0.049,\ \Omega_r \approx 10^{-4}$ traced to *zone-geometric inputs* (warp factors $A_\xi, A_\eta$, brane tension $\sigma$) — not fit | 08-FRIEDMANN_EVOLUTION §2.4; Vol 1 Ch 6 §6.7 | §8.6 |
...
- **Vol 1 Ch 5** — Firmament as codimension-2 brane $\Sigma = Z_{2.2}$ with tension $\sigma > 0$, mass density $\mu$, wave speed $c^2 = \sigma/\mu$ (Eq 1.5.37); junction conditions.
...
- **Vol 5 Ch 7** — Singularity resolution; brane-nucleation event replaces the Big Bang singularity. The boundary data for the past timelike geodesics in this chapter come from Theorem 5.7.3.

## "Why" Chain

1. **Why does cosmology have anything to derive?** Because once we have the EFE on the brane (Vol 5 Ch 1) and the bulk Waters fields as the dominant matter content (Vol 1 Ch 6), the *largest-scale* solutions of the EFE are determined: the cosmological principle (homogeneity, isotropy on scales $\gtrsim 100$ Mpc) is a *consequence* of zone symmetry (Vol 1 Ch 4 §4.7), not a postulate.
2. **Why FLRW and not some more general metric?** Because zone symmetry forces the brane induced metric to be conformally flat in the spatial slice on cosmological scales; FLRW is the unique homogeneous-isotropic solution and falls out, not in.
...
5. **Why does $\rho_B$ scale as $a^{-3}$ instead of as a constant?** Because Waters Below is in its broken phase with VEV $\langle\Psi_B\rangle = v_B$, but the *projected number density* of $\Psi_B$ quanta on the brane dilutes geometrically as the brane expands — exactly like nonrelativistic dust.
6. **Why is the universe spatially flat?** Because the 4D induced metric inherits its spatial curvature from the bulk extrinsic curvature of the brane in the $(\xi,\eta)$ directions, and Vol 1 Ch 6 §6.5 shows that the equilibrium brane has $K^{(\xi)} = K^{(\eta)} = 0$ on cosmological averages — so $k = 0$ is forced by zone equilibrium, not assumed.
7. **Why is the energy budget exactly 68/27/5?** Because the warp factors $A_\xi, A_\eta$ and the brane tension $\sigma$ have determinate values from Vol 1 Ch 6 §6.7. The ratios are *geometric*, not phenomenological.
...
|---|---|---|---|---|
| D1 | Cosmological-symmetry reduction of the brane induced metric | Vol 1 Ch 4 §4.7 (zone symmetry); brane embedding (Vol 1 Ch 5) | The induced 4D metric on the brane is FLRW: $ds^2_{4D} = -c^2 dt^2 + a^2(t)\,d\Sigma_k^2$ | Lemma 5.8.1, Eq (5.8.4) |
| D2 | First Friedmann equation from $tt$-component of EFE on FLRW brane | Vol 5 Eq (5.1.34) (EFE); D1; Waters stress-energy from Vol 1 Ch 6 | $H^2 = (8\pi G_4/3)\sum_i \rho_i - kc^2/a^2$ | Theorem 5.8.1, Eq (5.8.13) |
...
|---|---|---|---|---|---|
| Fig 5.8.1 | Brane FLRW: how zone symmetry reduces the 6D metric | §8.2, after Eq (5.8.4) | Schematic | The 6D bulk with $(\xi,\eta)$ extra dims, the brane $\Sigma$ as a 4-slice, and the cosmological-symmetry orbits on the brane that force the FLRW form | Spatial relationship: 6D ↔ 4D ↔ FLRW reduction is hard to follow in prose alone |
| Fig 5.8.2 | The Waters fields as cosmological fluid: bulk profile → brane stress-energy | §8.3, after Eq (5.8.10) | Diagram | $\Psi_A(\xi)$ profile in the $\xi$-direction (constant $V_A$ at the minimum), $\Psi_B(\eta)$ profile in the $\eta$-direction (VEV $v_B$ in the broken phase), and the projection arrows down to the brane stress-energy $T_{\mu\nu}^{(A)}, T_{\mu\nu}^{(B)}$ | Multi-step transformation: profile → KK projection → 4D fluid identification |
...
5. **Conceptual.** Where in the derivation of the Friedmann equation is the assumption that the brane is spatially flat ($k = 0$) used? What would change if $k = +1$ or $k = -1$?
...
|---|---|---|
| G1: The warp-factor profiles $A_\xi, A_\eta$ that determine the $\Omega$'s are themselves derived in Vol 1 Ch 6 §6.7, but the *normalization constants* $\xi_A, \gamma$ are matched to the observed brane radius, not derived from a more fundamental scale | LOW | Cite Vol 1 Ch 6 §6.7 explicitly. The chapter does not claim to derive $\xi_A, \gamma$; it claims that, given $(\xi_A,\gamma)$, the *ratios* $\Omega_A : \Omega_B : \Omega_b$ come out as observed. This is what the Reviewer's Ledger calls an Inheritance, not a Derivation. |
...
| G3: The "perfect fluid" approximation for $\Psi_B$ on the brane requires that the field has equilibrated to its broken-phase VEV; this equilibration is itself a Vol 3 Ch 8 phase transition not re-derived here | LOW | Cite Vol 3 Ch 8 explicitly. |
