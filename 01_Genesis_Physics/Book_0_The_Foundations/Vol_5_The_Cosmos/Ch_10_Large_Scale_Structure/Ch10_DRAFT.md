...
### §10.1.1 From Vol 5 Ch 8 — the brane FLRW background
...
with $\Omega_A = 0.684$, $\Omega_m = \Omega_b + \Omega_B = 0.049 + 0.266 = 0.315$, $\Omega_r \approx 9.2 \times 10^{-5}$, all from Ch 8 §8.6.3 and inherited there from the bulk-field calculation of Vol 1 §6.7. The chapter writes $E(a) = E(z = 1/a - 1)$ interchangeably.
...
### §10.1.3 From Vol 1 Chs 5–6 — the Firmament brane and the Waters fields

Vol 1 Ch 5 established the Firmament as a codimension-2 brane $\Sigma$ with tension $\sigma > 0$ and mass density $\mu$, supporting transverse waves at speed $c^2 = \sigma/\mu$ (Vol 1 Eq 1.5.37). Vol 1 Ch 6 introduced the Waters Above ($\Psi_A$, a bulk field with equilibrium energy density $\bar\rho_A$ that contributes equation of state $w_A \approx -1$ in the brane projection) and the Waters Below ($\Psi_B$, a brane-localized field with equilibrium energy density $\bar\rho_B$ that contributes equation of state $w_B \approx 0$ and self-coupling many orders of magnitude weaker than electromagnetism).
...
### §10.1.6 From Vol 5 Ch 1 — perturbed Einstein equations on the brane

Vol 5 Ch 1 derived the linearized Einstein equations on the brane (Eq 5.1.45) and the Newtonian limit on subhorizon scales (Eq 5.1.55), in which the metric perturbation $\Phi$ couples to the matter density $\delta_m$ through the Poisson equation $\nabla^2\Phi = 4\pi G \bar\rho_m a^2 \delta_m$. This is the same Poisson equation that appears in (5.10.5), and the chapter will use it without re-derivation.

---

## §10.2 The Setup: A Density Perturbation on the Brane FLRW Background

Before deriving the growth equation we need to be specific about *what* is fluctuating. On the brane FLRW background of chapter 8, the matter density is

$$(5.10.6)\quad \rho_m(\vec x, t) = \bar\rho_m(t)\,[1 + \delta_m(\vec x, t)],\qquad \bar\rho_m(t) = \bar\rho_b(t) + \bar\rho_B(t),$$

where the bar denotes the spatial average and $\delta_m$ is the *matter density contrast*. The components of $\bar\rho_m$ are the baryons $\bar\rho_b$ and the brane-projected Waters Below field $\bar\rho_B$ — the two components of the chapter-8 inventory whose equation of state is $w \approx 0$ on the brane and that therefore behave as non-relativistic matter for the purposes of the gravitational collapse problem. The radiation $\bar\rho_r$ and the Waters Above $\bar\rho_A$ are *not* part of $\bar\rho_m$; we will see in §10.4 that they enter only through the background $H(a)$.

It is worth being explicit about what $\delta_m$ is *not*. It is not a perturbation of the bulk Waters Above field — that field, by §10.1.3, has no subhorizon perturbations to leading order. It is not a perturbation of the brane geometry — at the scales of interest the brane is rigid and the metric perturbation is captured entirely by the Newtonian potential. It is not a perturbation of the radiation field — radiation has its own continuity equation with a different equation of state and after recombination decouples from the matter.

We work in the comoving Newtonian gauge on subhorizon scales. The relevant length scales are bounded above by the Hubble radius $c/H_0 \approx 4{,}450$ Mpc (above this, full GR perturbation theory is required, but the load-bearing observable scales — $k > 0.001$ Mpc⁻¹, equivalently $\lambda < 6{,}300$ Mpc — sit safely below this) and bounded below by the brane thickness $\sim 10^{-19}$ m (well below any cosmological scale). At every scale of interest, the Newtonian limit of (5.10.5) and the Poisson equation $\nabla^2\Phi = 4\pi G \bar\rho_m a^2 \delta_m$ are exact to the precision of the data.
...
What the framework adds to the calculation is the *interpretation* of $\bar\rho_m$. In $\Lambda$CDM, $\bar\rho_m = \bar\rho_b + \bar\rho_\text{CDM}$, where $\bar\rho_\text{CDM}$ is a cold dark matter component whose origin is, by construction, an open question. In the framework, $\bar\rho_m = \bar\rho_b + \bar\rho_B$, where $\bar\rho_B$ is the brane-projected Waters Below field of Vol 1 Ch 6, whose properties (weak self-coupling, non-relativistic dispersion, conserved number) were derived from the brane field equations, not posited. In the linear-regime growth calculation the two interpretations are interchangeable; in the next chapter (Ch 11) they begin to differ.
...
By Vol 1 §6.4, the Waters Below field $\Psi_B$ on the brane has equation of state $w_B \approx 0$, sound speed $c_{s,B} \ll c$ (specifically, $c_{s,B}/c$ is set by the brane field equations and is many orders of magnitude below unity), and self-coupling many orders of magnitude weaker than electromagnetism. These three properties — non-relativistic motion, negligible pressure, weak self-interaction — are exactly the defining properties of cold dark matter:
...
In the linear-perturbation theory, this means: the brane-projected Waters Below density $\bar\rho_B$ is added to the baryon density $\bar\rho_b$ to form the matter density $\bar\rho_m$ in (5.10.6) and (5.10.7). The growth equation does not need to be modified. The clustering of Waters Below is the clustering of cold dark matter, and the growth factor (5.10.10) is the standard growth factor.
...
Consider a spherically symmetric overdensity in an otherwise uniform expanding background. By Birkhoff's theorem (which Vol 5 Ch 1 derived as a consequence of the brane Einstein equations), the dynamics of the overdense region depend only on its enclosed mass and not on the matter outside. The overdense region therefore evolves as if it were a closed mini-universe with its own Friedmann equation. If the overdensity is large enough — specifically, if the corresponding linear-theory $\delta$ extrapolated forward in time exceeds a critical value $\delta_c$ at the moment of collapse — the mini-universe reaches a turnaround radius, recollapses, and virializes at a final overdensity
...
Beyond $k_\text{NL}(z)$, the chapter has *nothing quantitative to say*. The framework does not yet have a brane-aware $N$-body solver. The simulator `structure_formation.py` runs linear theory plus Press–Schechter; it does not solve the Vlasov–Poisson system. A full nonlinear treatment would require:

- a brane-aware $N$-body code that respects the $\Omega_A, \Omega_B$ split rather than treating them as $\Lambda$CDM dark energy and dark matter (a non-trivial code project, since the standard $N$-body solvers hard-code the $\Lambda$CDM background and would need to be extended);
...
A working reference implementation of these eight steps (in 28 lines of Python with `numpy` and `scipy.integrate`) is included as an appendix to this chapter; the Student reviewer should be able to run it on a laptop in under one second and verify $\sigma_8 = 0.811$.
...
|---|---|---|---|---|
| L1 | The linear growth equation $\ddot\delta_m + 2H\dot\delta_m - 4\pi G\bar\rho_m\delta_m = 0$ | Derivation | §10.3, Eq (5.10.7) | Derived from Vol 3 Ch 5 fluid equations on the brane FLRW background |
...
1. *Linear growth solver.* Solve the linear growth equation (5.10.8) numerically for the framework's $\Omega_i$ ($\Omega_A = 0.684$, $\Omega_m = 0.315$, $\Omega_r = 9.2\times 10^{-5}$) from $a = 10^{-3}$ to $a = 1$ using a 4th-order Runge–Kutta integrator and the chapter-8 $E(a)$. Verify that $D^+(a=1)/D^+(a=0.1) \approx 7.7$ as in Table 5.10.1.
