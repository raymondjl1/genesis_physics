...
- **R6–R8 (Physicist's notes on linearization, field stability, and brane-tension precision).** These are largely absorbed into the new R1, R2, R5 paragraphs and into the existing §4.11.3 conjecture enumeration. Further elaboration would push the chapter past its word budget without adding content the reader cannot already reconstruct.
...
3. Massive-particle dimensional-bypass energy range ($10^{20}$ uncertainty from $\Delta\eta$) → Vol 6 "Microstructure of the membrane"
