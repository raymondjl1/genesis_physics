...
- **Theorem 5.6.3 (6D Unitarity):** Constructs the 6D Hamiltonian from Vol 1 Ch 5 (Firmament) + Vol 1 Ch 6 (bulk) + junction coupling, shows essential self-adjointness via Reed–Simon, and applies Stone's theorem to obtain unitary evolution on the full Hilbert space $\mathcal H_\text{total} = \mathcal H_\text{Firm} \otimes \mathcal H_\text{bulk}$. The Firmament-only density matrix evolves from pure to mixed through entanglement, not loss.
...
2. **Theorem 5.6.3 proof (Physicist):** Added parenthetical pointer to Vol 0 Appx A.7 for the Stone's-theorem form used, plus a parenthetical explaining that the required common dense domain of Firmament, bulk, and interaction Hamiltonians follows from Vol 1 Ch 6 §6.4.
...
- Vol 1 Ch 6 §6.4–§6.5 (bulk fields, junction coupling, Firmament–bulk tensor factorization)
