...
- **Why entry point.** The standard Hawking paradox says a pure state becomes mixed — violating unitarity. Vol 3 Ch 12 told us information cannot be destroyed; Ch 5 showed the Firmament has a hole; we now show these are consistent.
- **What this chapter does:** derive Hawking radiation from Firmament membrane dynamics; state the conventional paradox as a no-go theorem; show the zone framework's premises break the theorem; prove 6D unitarity; prove the Page curve.
...
### §6.1.1 From Vol 1 Ch 5 — Firmament membrane dynamics
...
- Coupling to the Firmament through a boundary term $\mathcal L_\text{int} = \lambda\,\Psi_\text{WB}(\gamma)\cdot\mathcal O_\text{Firm}(\gamma)$.
- Key fact: the bulk has physical degrees of freedom that an on-Firmament observer cannot measure directly.

### §6.1.3 From Vol 1 Ch 11 — Entropy as Firmament-mode counting + Liouville on 6D phase space
...
### §6.1.5 From Vol 4 Ch 6–9 — QFT on the Firmament
...
- Zone UV cutoff $\Lambda_{\mathrm{zone}} = \hbar c/\eta_B \approx 0.152\,\mathrm{GeV}$ from Vol 4 Ch 8 (corrected per CT-4.Λ, Rev. 2026-05-15; QCD scale, not Planck scale).
...
- **(c) Firewalls (AMPS 2012).** Fails equivalence principle for infalling observers; Ch 5 §5.5.3 already noted why the Firmament picture gives no firewall.
...
## §6.3 Hawking Radiation From Firmament Dynamics — ~1600 words

### §6.3.1 The setup: a scalar Firmament mode near the breach
Consider a massless scalar Firmament mode $\phi(t,r,\theta,\varphi)$ obeying the wave equation
...
- **Horizon-modes:** waves on the Firmament surface at $r^* \to -\infty$ (the breach edge). These live only for $t < t_\text{breach}$.
...
The Bogoliubov coefficient is determined by the analytic continuation of the out-mode around the turning point $r = r_s$. In the Firmament framework this is the point where $v^2(r) = 0$ — a physical turning point, not a coordinate one. Using standard WKB-plus-analytic-continuation (the same technique used in Vol 4 Ch 7 for $S$-matrix branch cuts), the ratio is:
...
Consistency check: Ch 5 got (5.5.24) thermodynamically; we now got (5.6.7) microscopically. The equality is not a coincidence — it follows from the first law together with the fact that the Bogoliubov derivation *uses* the fact that the tension vanishes at $r_s$ (Firmament framework) or that the Killing vector becomes null at $r_s$ (standard GR). These are the same surface.
...
Hawking radiation is not a mystery in the zone framework — it is a Bogoliubov transformation on Firmament modes in a region where the Firmament wave speed vanishes. The temperature is forced by the surface gravity of the breach. **The same derivation in pure GR is the standard one, and produces the same answer.** What the Firmament membrane version adds is (a) a *physical* turning point rather than a coordinate one, and (b) the essential ingredient for what follows — the bulk, with its own Hilbert space, sitting right there at the turning point.
...
The weaker statement: $S \le k_B A/(4\ell_P^2)$ for any region bounded by area $A$. Also saturated by black holes. In the zone framework, this is simply the statement that the Firmament boundary counts its own modes, and (5.5.20) is the natural ceiling.

### §6.4.3 Why the bounds hold — zone-architecture reading
In standard GR, the holographic bound is mysterious: why should a 3D region have its information content bounded by a 2D quantity? In the zone framework, the answer is immediate: because the "information" that an exterior observer can extract lives on the *Firmament*, which has boundary degrees of freedom scaling as the area of the boundary. Bulk information is inaccessible to the Firmament observer by construction and does not enter the bound. The bound is a statement about *what a Firmament observer can know*, not about *what exists*.

### §6.4.4 This reframing matters
Because it tells us that the "information" in $S_\text{BH}$ is not the total information about the infallen matter. It is the information a Firmament observer can extract from the breach boundary by watching the radiation. The infallen matter's full information lives in the combined Firmament+bulk state, and the bulk share is not bounded by $A/(4\ell_P^2)$ at all — it can be much larger, limited only by the bulk entropy density and the bulk volume.

### §6.4.5 Numerical feeling
For a solar-mass black hole: $S_\text{BH}\sim 10^{77} k_B$; the total 6D bulk entropy inside the breach (if estimated as $\rho_\text{bulk} V \approx $ Waters-Below density times breach volume) is at least comparable, possibly larger. The Firmament-boundary share is the *visible* part; the bulk share is the *hidden* part. Total information is preserved; only the visible share is rationed.

- **Exit condition.** Reader understands the entropy bounds as statements about Firmament-accessible information, not about total information.

## §6.5 Where the Information Goes: The 6D Resolution — ~1700 words (central section)

### §6.5.1 The Hilbert space decomposition
Define:
$$(5.6.8)\quad \mathcal H_\text{total} = \mathcal H_\text{Firm} \otimes \mathcal H_\text{bulk},$$
where $\mathcal H_\text{Firm}$ is the Fock space of Firmament-mode occupations (Vol 4 Ch 6) and $\mathcal H_\text{bulk}$ is the Fock space of Waters-Below (and Waters-Above) field excitations (Vol 1 Ch 6).

### §6.5.2 The 4D effective theory is a reduction
A Firmament-bound observer (i.e., anyone at $r > r_s$) has access only to observables in $\mathcal H_\text{Firm}$. Their density matrix is obtained by tracing over $\mathcal H_\text{bulk}$:
$$(5.6.9)\quad \rho_\text{Firm}(t) = \text{tr}_\text{bulk}\,\rho_\text{total}(t).$$
Even if $\rho_\text{total}$ is pure, $\rho_\text{Firm}$ is generally mixed. The mixing is *not* information loss — it is entanglement with an inaccessible subsystem, exactly the same as in ordinary thermodynamics (Vol 3 Ch 12 §12.5).

### §6.5.3 Theorem 5.6.2: The Mathur theorem does not apply to the zone framework
**Lemma 5.6.2.** *Premise M1 of Theorem 5.6.1 (exterior Hilbert space is complete) is violated in the zone framework, because the bulk Hilbert space is separately populated and is causally connected to the Firmament through the boundary coupling term of Vol 1 Ch 6.*

**Proof.** From Vol 1 Ch 6 Eq. (1.6.21), the Waters-Below field has an independent set of creation operators $\hat b^\dagger_k$ with $[\hat a_k, \hat b^\dagger_{k'}] = 0$. Therefore $\mathcal H_\text{Firm}$ and $\mathcal H_\text{bulk}$ are distinct Hilbert-space factors; the Firmament alone is not the full Hilbert space. $\square$
...
Infalling matter deposits information into $\mathcal H_\text{bulk}$ as Waters-Below field configurations (the $\Psi_\text{WB}$ mode amplitudes). Bulk-to-Firmament coupling (via the junction condition at the breach edge) transfers those amplitudes back to outgoing Firmament modes during the emission process. Over the lifetime of the black hole, every bit that fell in is emitted once; cumulatively, the full state $\rho_\text{total}$ remains pure throughout.
...
2. **The bulk is causally connected to the Firmament via an explicit coupling term.** The "how does the information come back out?" question has a specific answer: through the boundary coupling.
...
Let $D_R(t) = $ dimension of the Hilbert space of emitted radiation at time $t$, and $D_B(t) = $ dimension of the Hilbert space of the remaining (Firmament-breach + bulk-Waters-Below-inside) system at time $t$. Initially $D_R(0) = 1$, $D_B(0) = e^{S_\text{BH,initial}}$. As radiation proceeds, $D_R$ grows and $D_B$ shrinks.
...
**Theorem 5.6.4 (Page Curve).** *Assuming 6D unitarity (Theorem 5.6.3) and Haar-random initial entanglement between Firmament and bulk, the entanglement entropy of the emitted Firmament radiation is*
...
Important: Page's argument *requires* that the "black-hole side" of the tensor factorization has dimension $D_B(t)$ that shrinks as radiation proceeds. In pure 4D, without the bulk, the "black-hole side" is... what? The singularity? An empty set? Pure 4D cannot answer this without firewalls or fuzzballs or an invocation of Planck-scale magic. The zone framework has a concrete answer: the "black-hole side" is the combined (shrinking Firmament patch + shrinking bulk region inside the breach), and both shrink smoothly as the breach closes.
...
### §6.9.1 Derivations
- Bogoliubov coefficients (5.6.3)–(5.6.6) → Hawking spectrum (5.6.7) — from Ch 5 tension profile + Firmament wave equation.
...
### §6.9.7 Forward links
- Ch 7 (Singularity Resolution) will use the Firmament-reopening dynamics previewed at the endpoint.
