...
| R8 | **Zone-architecture predictions beyond standard GR:** scalar breathing mode from the internal-space radion; brane-tension correction to orbital decay; explicit sensitivity floors at which each would become visible to current and planned detectors | §3.9 |
| R9 | Honest scorecard: every predicted GW150914 quantity listed with predicted value, observed value, and error bar | §3.10 |
| R10 | Forward dependency link to Ch 5 (black holes as zone infrastructure) clearly flagged: the ringdown of §3.7 assumes a Kerr exterior, which Ch 5 will justify at the membrane level | §3.10 |
