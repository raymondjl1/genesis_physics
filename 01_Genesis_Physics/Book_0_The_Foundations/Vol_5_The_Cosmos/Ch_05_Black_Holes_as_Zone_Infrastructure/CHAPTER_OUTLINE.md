...
- Key equations: (5.1.34) Schwarzschild, (5.1.36) Kerr, (1.5.37) $c^2 = \sigma/\mu$, (1.5.28) membrane stress-energy, (1.5.17) extrinsic curvature.
...
- Why: because $c$ is the wave speed on the membrane (Vol 1 §5.3), and gravitational redshift of $c$ requires a corresponding redshift of $\sigma$ (since $\mu$ is set by local bulk density, which varies slowly with $r$).
- Key derivation:
...
- Key derivation:
  ...
  3. Conclude: the membrane *breaches* at $r = r_s$. The region $r < r_s$ is not membrane at all.
  4. Physical picture: at the breach, the Firmament's 3-brane structure opens into the bulk. The normal bundle of the membrane becomes degenerate at $r = r_s$; the 6D hypersurface $Z_{2.2}$ is no longer a smooth submanifold there.
...
- Why: because the horizon is where the membrane ends; in the standard framework it is where $g_{tt} \to 0$, and in both frameworks it is at $r = r_s$.
- Key content:
  1. Derivation that null geodesics at $r = r_s$ point "into" the breach (the induced metric on the membrane becomes degenerate).
  2. Reinterpretation of gravitational time dilation: the membrane vibration rate slows asymptotically as the tension drops.
  3. Smooth passage in the freely-falling frame — the breach is *not* a firewall.
  4. Inside the horizon: what was Zone $Z_{2.2.2}$ (brane) is now the boundary of Zone $Z_{2.2.1}$ (Waters Below). The temporal direction is no longer a Firmament coordinate.
...
- Key content:
  1. State the Vol 1 Ch 11 thermodynamic axiom: the entropy of a region of the Firmament is proportional to the number of brane-vibration modes that live there.
  ...
  4. Entropy $S = k_B N_\text{modes}/4$ (the factor 4 comes from the normalization of the brane mode inner product — briefly justified, detailed in Ch 11 §11.6).
...
- Challenge: derive $\rho_\text{crit}$ purely from membrane mechanics without assuming $r_s = 2GM/c^2$ a priori; compute the breach-mode spectrum under a leaky boundary condition.
