...
This last point deserves emphasis: **charge is extra-dimensional momentum.** A particle with electric charge $q$ is, from the 6D perspective, a particle with momentum $p_\xi \propto q$ in the $\xi$ direction. An electrically neutral particle has zero $\xi$-momentum. This identification — first made by Klein in 1926 for the original 5D theory — extends naturally to all gauge charges. Color charge in QCD corresponds to a specific mode of motion at the zone boundaries. Weak isospin corresponds to motion in the $\eta$ direction near the Waters Below.
...
The reason we don't observe these large extra dimensions is not that they are small, but that the warp factors (1.4.2) confine matter and light to the Firmament. We live on a brane — a membrane in the higher-dimensional space — and our physics is the shadow of the bulk geometry onto that brane.
...
The fine structure constant — one of the most mysterious numbers in physics — emerges as the logarithm of a geometric ratio. The geometric factor $K \approx 1.44$ is not a fitted parameter — it is derived from the overlap integral of the electromagnetic zero mode with the bulk warp factors (Eqs. 1.4.61a–1.4.61b in Volume 1). With this derived value, $\xi_A / \eta_B \approx 2.3 \times 10^{41}$, the prediction is $\alpha^{-1} \approx 137.1$, compared to the experimental value $137.036$ (Eq. 1.4.64–1.4.65). Agreement to better than 0.1%.
...
This means the number of forces is insensitive to the detailed values of the warp factors, the brane tension, the cosmological constant, or any other continuous parameter. You can adjust $\xi_A$, $\eta_B$, $\sigma$, and $\Lambda_6$ freely — the force strengths will change, but the number of forces will remain four. Only a change in the *topology* of the extra dimensions (adding a dimension, changing the number of zones, or modifying the boundary conditions qualitatively) could alter the force count.
...
It is worth pausing to appreciate the economy of this result. The Standard Model has approximately 19 free parameters (coupling constants, masses, mixing angles). In the zone manifold framework, all 19 are in principle calculable from the zone geometry — from the warp factors $A(\xi,\eta)$ and $B(\xi,\eta)$, the brane tension $\sigma$, and the 6D gravitational coupling $G_6$. We say "in principle" because several of these calculations are deferred to later volumes (the fermion masses to Volume 4, the mixing angles to Volume 5). But the claim is clear: **the zone manifold has fewer free parameters than the Standard Model.** This is the mark of a deeper theory — it explains more with less.
...
- **Chapter 3: Electromagnetism from Membrane Wave Propagation.** Derive all four Maxwell's equations from Firmament vibrations. Show that c is a membrane property. Begin the fine structure constant derivation.
