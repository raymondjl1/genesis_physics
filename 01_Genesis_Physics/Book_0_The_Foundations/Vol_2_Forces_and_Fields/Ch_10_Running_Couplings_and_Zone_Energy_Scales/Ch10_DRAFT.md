...
This is the membrane scale — approximately 1 GeV, squarely in the nuclear physics regime. Below this energy, the extra dimensions are invisible and physics looks four-dimensional. Above it, the running of couplings encodes the geometry of the ξ and η directions.

### The UV and IR Cutoffs

The running has two natural boundaries imposed by the zone architecture.

**The UV cutoff** comes from the Firmament tension. At sufficiently high energy, the membrane tension σ dominates all dynamics, and the concept of individual particles breaks down. This occurs at the Planck scale:
...
From the zone architecture, the coupling constants at the membrane scale Q_m ≈ 1 GeV take their "bare" values before running. These were derived in Chapters 3, 6, and the research derivation 10-COUPLING_CONSTANTS_DERIVATION.md:
...
This is the result of the 2D logarithmic Green's function in the extra dimensions, derived in Chapter 3, equation (2.3.63). The membrane-scale value is the "natural" coupling before any running.
...
For the electromagnetic coupling specifically, the one-loop running from the membrane scale is:
...
Starting from the membrane-scale value (2.10.24) and running to M_Z using the full Standard Model matter content:
...
**Honest assessment:** The beta function coefficients are derived exactly from the zone Lagrangian structure and match the Standard Model values perfectly. The running equations at one loop are identical. The discrepancies in numerical running arise from: (a) the precision of the membrane-scale anchor values, and (b) two-loop and higher corrections not yet computed within the full 6D framework.
...
3. **The membrane-scale anchor values** (2.10.24-2.10.26) as boundary conditions
