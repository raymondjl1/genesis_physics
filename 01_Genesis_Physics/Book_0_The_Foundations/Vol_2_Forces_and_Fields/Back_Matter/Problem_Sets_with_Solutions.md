# Problem Sets with Solutions — Foundations Vol 2: Forces and Fields (Back Matter Edition)

> **This is the back-matter, with-solutions edition of the Vol 2 problem sets.** It is the companion to `../Problem_Sets.md` and contains **exactly the same 88 problems** (8 per chapter × 11 chapters) in the same numbering. The two files are kept consistent: the problem statements here are identical to the main set, and the same 33 worked solutions (~38%, three per chapter) appear in the *Selected Solutions* section. Problems marked **[SOLUTION PROVIDED]** are worked in full at the end; the remainder are left as exercises with their target results stated in the problem text.
