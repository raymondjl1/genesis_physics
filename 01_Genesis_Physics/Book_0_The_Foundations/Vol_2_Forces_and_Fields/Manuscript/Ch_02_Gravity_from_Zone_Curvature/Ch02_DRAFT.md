...
> **Warp profile note:** The Waters Below warp profile adopted in this chapter is the exponential form $B_\eta(\eta) = B_0 - \frac{\gamma}{2}\eta$. Chapter 4 uses a different functional form for the same dimension: $B(\eta) = -\gamma^2\eta^2/2$ (Gaussian). **OP-2.WP RESOLVED (2026-05-15):** The canonical form is $B_\eta \approx \text{const}$ (RS-type, flat leading-order), derived from the variational equation for $\Psi_B$ + ηη Einstein equation. The Gaussian form is retained as a sub-leading approximation valid only when $V_B$ is harmonic near its minimum. METRIC\_6D\_SOLUTIONS.md §3.4.1 updated. See `Research/Foundations/B_ETA_WARP_RESOLUTION_OP2WP.md`.
