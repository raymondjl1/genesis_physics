...
> **⚠ Mathematical Correction (Rev. 2026-05-14) — Z₃ Orbifold on Real Coordinate:**
> As written, the identification $\eta \to e^{2\pi i/3}\eta$ is undefined because $\eta \in \mathbb{R}$ is a real coordinate and $e^{2\pi i/3}$ is complex. Rotating a real number by a complex phase does not yield a real number, so this action cannot be an isometry of the real line.
>
> The correct construction requires the Waters Below extra dimension to have a *two-dimensional* fiber. Let $w = \eta_1 + i\eta_2 \in \mathbb{C}$ be a complex coordinate built from two real extra-dimensional coordinates $(\eta_1, \eta_2)$ that together parametrize the Waters Below transverse directions. The $\mathbb{Z}_3$ action is then:
>
> $$w \;\mapsto\; e^{2\pi i/3}\, w \tag{Z3-corr}$$
>
> which is a well-defined rotation in the $(\eta_1, \eta_2)$ plane by $120°$. The boundary $|w| = \eta_B$ is a circle in this 2D fiber, and the quotient $S^1/\mathbb{Z}_3$ (or more precisely the $\mathbb{Z}_3$ orbifold of the disk $|w| \leq \eta_B$) divides the fiber into three fundamental domains. The warp factor $B(w, \bar w) = -\gamma^2 |w|^2/2 = -\gamma^2(\eta_1^2 + \eta_2^2)/2$ is manifestly $\mathbb{Z}_3$-invariant because $|e^{2\pi i/3}w| = |w|$.
>
> The orbifold condition on wavefunctions becomes:
>
> $$\psi(e^{2\pi i/3} w) = e^{2\pi i n_c/3}\,\psi(w), \quad n_c \in \{0,1,2\}$$
>
> This is mathematically precise. The derivation in this section proceeds as written *once this complexification is understood*. The three topological sectors correspond to the three irreducible characters of $\mathbb{Z}_3$ acting on $\mathbb{C}$, and these sectors generate $SU(3)_C$ by the argument in the next subsection. The correction affects the rigorous justification of the construction, not the final physical conclusions. The complete derivation with two-dimensional Waters Below fiber will appear in Vol 4.
>
> **RT-2.SU3 is now closed (2026-05-15).** The rigorous derivation of SU(3)_C from the Z₃ orbifold on the 2D complex fiber w = ξ+iη = ρe^{iψ} (ψ ~ ψ+2π/3) is documented in `Research/Foundations/RT2_SU3_Z3_ORBIFOLD.md`. The three Z₃ irreducible representations → three color sectors; U(3) projected to SU(3) by the tracelessness condition; 8 generators (2 diagonal + 6 off-diagonal) = 8 gluons. The derivation here is physically correct and is now supported by the completed rigorous construction. See also Ch06 §6.4.2–6.4.3 for the canonical treatment using polar coordinates.

This topological structure is profound. A wavefunction in the Waters Below must be single-valued when we traverse the $\eta$ circle and return to the starting point. But the orbifold identification says that $w$ and $e^{2\pi i/3} w$ are the same point in the 2D fiber (see correction box above). For a wavefunction to respect this, it must satisfy:
