...
> **Structural reminder.** *Firmament* (and, where they appear later, *Waters Above / Waters Below*) are the structural objects derived in Vol 1 Ch 3–5 from Genesis 1:6–8 (see Vol 2 Ch 1 §1.0 sidebar). Not metaphor — load-bearing geometry. (For the Hebrew word study behind *rāqîaʿ* / *mayim* and the separation verb, see Vol 1, Appendix C, "Hebrew Analysis.")
...
A word on where this gravitational sector sits in the zone architecture. Although this chapter speaks mostly in the language of "extra dimensions" and the "4D effective theory," the structure-forming, gravitationally binding role of gravity is anchored specifically in the Waters Below (zone Z₂.₂.₁), which the reference architecture identifies as providing the "gravitational scaffolding" of the cosmos (Gen 1:2; `Quality_Control/Reference/Zone_Architecture.md`, Tables 1 and 3). The dark-matter contribution to $T_{\mu\nu}$ and the binding that builds large-scale structure both trace to this zone; its companion, the Waters Above (Z₂.₂.₃), supplies the $\Lambda_\text{eff}$ term above. Naming the zones here keeps the gravitational sector tied to the same scaffolding the rest of the volume invokes.
...
In the 6D zone action, the warp factors $A(\xi,\eta)$ and $B(\xi,\eta)$ are dynamical fields — they are the solutions to the 6D Einstein equations with Waters field sources (Vol 1, Ch 4; Ch 2, §2.1). The Waters fields $\Psi_A$ and $\Psi_B$ that source them are not abstract moduli alone: they are the geometric content of the Genesis waters — the *mayim* above and below that the firmament divides (Gen 1:7), set in motion by the Spirit that "moved upon the face of the waters" (Gen 1:2, *rûaḥ ʾĕlōhîm məraḥep̄eṯ*), as Vol 1 establishes. The breathing mode below is, in that sense, the residual oscillation of those waters. In the KK reduction to 4D, the zero modes of these warp factors become scalar fields — the *moduli* — that couple to the 4D gravitational sector. When moduli are stabilized (as they are in the current Fall thermodynamic phase, Vol 1 Ch 6), they take fixed background values, and their fluctuations are massive. But they are not infinitely massive: their mass is set by the stabilization potential, which is finite.
