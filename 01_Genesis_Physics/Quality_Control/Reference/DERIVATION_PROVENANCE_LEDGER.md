...
| **Postulate F — winding n_w=3** | Waters-Above vortex sector carries winding 3 → Kähler spinors (spin-½) + twisted index +3 (three generations; **computed**, #851 fixed). | **— no verse** (topological). Direct Trinity→3: resonance only. **Z₃+energetics mechanism: REFUTED** (2026-06-12 two-team race + referee, convergent no-go: equivariant index equidistributes; quantization step empty off-apex; sector winding superselected — `Peer_Review/849_nw3_derivation_race/`). **Sharpened frontier (#849):** derive **n_q = 3** for the fiber background — then the equivariant index *automatically* gives 3 generations × 3 colors (9 = 3×3) from the one color-grounded Z₃. | **AXIOM** (irreducible, honest) → frontier **#849** |
