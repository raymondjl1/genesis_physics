...
## ☆ GOVERNING PROVENANCE PRINCIPLE (2026-06-11, author) — the standard everything is held to
**Everything must be derived from the framework's axioms and the Bible.** The only permitted foundations are (a) the zone-architecture **axioms** (themselves motivated by Genesis 1 — Λ_Z0 / the Godhead zone, the Waters duality, the membrane/Firmament, the metric-discontinuity and Fall transitions, Postulate F, etc.) and (b) **Scripture**. Every physical result must trace through a chain back to those. **A free-floating numerical fit, a "calibrated coefficient," or a bare measured input that is *not* derived from an axiom is — by this standard — an OPEN PROBLEM, not a result,** and must be (i) labeled honestly as such and (ii) tracked as a board issue until it is either derived from the axioms or shown to be one of them. This principle governs the whole registry and supersedes any "good enough fit" framing. Known items that do **not yet** meet it (the live frontier): n_w=3 (Postulate F) not yet derived from Z₀; Yukawa α (~20% fit); two-loop α→137.036; absolute mass scale / Higgs vev; g_W & λ_Higgs taken as measured inputs; the fine-structure C-coefficient corrections. These are board issues #846/#847/#848/#849/#850, not "done." **The full result-by-result provenance trace (Scripture → axiom → chain → result, each marked DERIVED / AXIOM / FIT) lives in `DERIVATION_PROVENANCE_LEDGER.md`.**
