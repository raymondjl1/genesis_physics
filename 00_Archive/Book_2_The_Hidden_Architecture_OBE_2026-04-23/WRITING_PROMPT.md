# Book 2: The Hidden Architecture — Writing Prompt (ARCHIVAL)

> **REPOSITIONING NOTE (April 2026):** This book's mandate has been absorbed by the **Popular Science Flagship** in `Book_1_Hidden_Architecture/`. This prompt is preserved for reference (voice intent, analogy discipline, audience) but should not be used to draft new standalone chapters in this folder. To draft popular-science material, use `../Book_1_Hidden_Architecture/WRITING_PROMPT.md` instead.
...
**Voice update (April 2026):** Under the franchise repositioning, the canonical voice for any popular-science chapter is Jeff L. Raymond's as defined in `../AUTHOR_VOICE_AND_BACKGROUND.md` — operator-not-professor, MBSE-disciplined, builder's honesty, quiet faith. "Brian Cox meets C.S. Lewis" captures the *tonal target* (wonder-driven, lucid, not sermonic) but not the author's actual credibility profile. Use the canonical voice when contributing material from this folder forward into the flagship.
...
```
Book_1_Hidden_Architecture/Manuscript/  ← PRIMARY SOURCE (every claim must trace here)
...
- **Every claim traces to Book 1 / Foundations.** Citation for any revived standalone use: "(For the mathematics behind this, see the Foundations Series, Vol X, Chapter Y.)" Note: the book formerly referenced here as *The Firmament Equations* was retitled in April 2026 to *The Hidden Architecture: A Physics of the First Page* (Book 1 flagship).
